...
【注释规范 - 加一原则】
- **加一原则**：在所有编程语言中，文档注释的标记 = 该语言行注释的最小单位重复一次。C 语言行注释 `//` 的最小单位是 `/`，文档注释即为 `///`。
- 使用中文编写注释，但是不要把英文技术术语翻译为中文。
- **二元切分原则**：注释只分两类，不得混用。
  1. **API 注释** — 必须用 `/// ` Doxygen 行注释。头文件（.h）中所有对外 API 的声明（包括类型、函数、宏和常量）必须使用 `/// ` 开头的连续行注释。注释至少包含 `@brief`；函数按需包含 `@param`、`@return`，额外说明使用 `@note`。
  2. **非 API 注释** — 必须用 `//` 或 `/* */`，禁止使用 `///`。适用于文件说明、章节分隔、实现细节、测试说明、static 内部函数说明以及函数体内的逻辑注释。函数体内部非直观逻辑（位操作、公式、状态机）必须使用行内注释解释。
- 头文件中的公开 API 注释是 API 使用方式的唯一事实来源。关键架构设计、设计决策和模块组织写入设计文档，不在文档中重复维护 API 清单。每个模块最多一个设计文档，放在 `doc/` 对应目录下。
