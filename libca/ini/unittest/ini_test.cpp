...
{
    auto document = read_ok("root = yes\n"
                            "[server]\n"
                            "host = 127.0.0.1\n"
                            "port: 8080\n");
...
{
    const std::string text = "# global comment\r\n"
                             "\r\n"
                             "[server]\r\n"
                             "; host comment\r\n"
...
TEST(IniIoTest, RoundTripsThroughFile)
{
    ...
    auto              document = read_ok("; file comment\n"
                                         "[app]\n"
                                         "name = libca\n");
...
TEST(IniReaderTest, InvalidUtf8ReportsParseErrorInsteadOfThrowing)
{
    ...
    auto raw = Utf8String::from_data_unchecked(reinterpret_cast<const ca::u8*>(gb), sizeof(gb) - 1);
