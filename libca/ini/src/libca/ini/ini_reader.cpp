...
{
    return arena.intern(ca::str::Utf8String(reinterpret_cast<const ca::u8*>(s.data()), s.size()));
}
...
{
    return (options.hash_comment && ch == '#') || (options.semicolon_comment && ch == ';');
}

// 在 value 部分中查找行内注释起始位置。考虑引号和转义。
// strict_whitespace=true 时要求注释符前是空白（或位于 value 起始）。
usize find_inline_comment(const std::string& value, const IniReaderOptions& options)
{
    ...
    for (usize i = 0; i < value.size(); ++i) {
        ...
        if (ch == '\'' && !in_double_quote && !escaped) {
            ...
            in_double_quote = !in_double_quote;
                 ...
                 (!options.inline_comment_strict_whitespace || i == 0 || is_space(value[i - 1]))) {
            return i;
        }
        escaped = false;
    }
...
bool detect_quotes(const std::string& value, char& quote_char)
{
    ...
    if ((first == '"' || first == '\'') && first == last && !trailing_quote_escaped(value)) {
        quote_char = first;
        return true;
    }
    return false;
...
void parse_line(ca::str::Utf8StringArena& arena, const std::string& raw,
                const std::string& line_ending, const std::string& current_section,
                const IniReaderOptions& options, usize line_number,
                ...
                LineParse& out)
{
    ...
    if (!ca::str::utf8_is_valid(reinterpret_cast<const u8*>(raw.data()), raw.size())) {
        ...
        out.parse_error.message  = ca::str::Utf8String::from_cstr("invalid UTF-8 in line");
        return;
    }
    ...
    if (trimmed[0] == '[') {
        ...
        if (end == std::string::npos) {
            ...
            out.parse_error.message =
                ca::str::Utf8String::from_cstr("section header misses closing ']'");
            return;
        }
    ...
    if (separator_pos == std::string::npos) {
        ...
        out.parse_error.location = SourceLocation{line_number, line_start_column};
        out.parse_error.message =
            ca::str::Utf8String::from_cstr("key/value line misses '=' or ':'");
        return;
    }
...
ca::Result<IniDocument, ParseError> IniReader::read(const ca::str::Utf8StringRef& input,
                                                    const IniReaderOptions&       options)
{
    ...
    for (usize pos = 0; pos <= text.size();) {
        ...
        usize       line_end   = pos;
        while (line_end < text.size() && text[line_end] != '\r' && text[line_end] != '\n') {
            ++line_end;
        }
...
ca::Result<IniDocument, ParseError> IniReader::read_file(const ca::str::Utf8StringRef& path,
                                                         const IniReaderOptions&       options)
{
