...
ca::str::Utf8StringArena& IniDocument::arena() noexcept
{
    return arena_;
}
...
const ca::str::Utf8StringRef* IniDocument::find_value(const ca::str::Utf8StringRef& section,
    ...
    const ca::str::Utf8StringRef& section, const ca::str::Utf8StringRef& key) const
{
    ...
    if (s.empty() || std::isspace(static_cast<unsigned char>(s[0])) || errno == ERANGE ||
        end != s.c_str() + s.size()) {
        return ca::Err(ca::str::Utf8String::from_cstr(
...
ca::Result<bool, ca::str::Utf8String> IniDocument::get_bool(const ca::str::Utf8StringRef& section,
                                                            const ca::str::Utf8StringRef& key) const
{
    const ca::str::Utf8StringRef* value = find_value(section, key);
    if (value == nullptr) {
...
void IniDocument::set(const ca::str::Utf8StringRef& section, const ca::str::Utf8StringRef& key,
                      const ca::str::Utf8StringRef& value)
{
    ...
    records_.insert(records_.begin() + static_cast<std::ptrdiff_t>(pos), std::move(key_record));
    rebuild_index();
}
...
bool IniDocument::remove(const ca::str::Utf8StringRef& section, const ca::str::Utf8StringRef& key)
{
    auto section_it = key_index_.find(section);
    if (section_it == key_index_.end())
        return false;
...
std::vector<ca::str::Utf8StringRef> IniDocument::keys(const ca::str::Utf8StringRef& section) const
{
    ...
    for (const auto& record : records_) {
        if (record.line.kind != IniLineKind::KeyValue)
            continue;
        const std::string_view rec_sec(reinterpret_cast<const char*>(record.line.section.data()),
