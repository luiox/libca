...
public:
    ...
    /// @brief 判断指定 key 是否存在。
    bool has(const ca::str::Utf8StringRef& section,
    ...
    ca::Result<ca::i64, ca::str::Utf8String> get_int(
        const ca::str::Utf8StringRef& section,
        const ca::str::Utf8StringRef& key) const;

    /// @brief 读取 double 值。自动剥首尾配对引号；解析失败返回错误。
    ca::Result<ca::f64, ca::str::Utf8String> get_double(
        const ca::str::Utf8StringRef& section,
        const ca::str::Utf8StringRef& key) const;

    /// @brief 读取 bool 值。接受 true/false/yes/no/on/off/1/0（大小写不敏感）。
    ca::Result<bool, ca::str::Utf8String> get_bool(
        const ca::str::Utf8StringRef& section,
        const ca::str::Utf8StringRef& key) const;
    ...
    bool remove(const ca::str::Utf8StringRef& section,
                const ca::str::Utf8StringRef& key);
