...
{
    auto result = JsonReader::read(
        Utf8StringRef::from_data(reinterpret_cast<const u8*>("\xEF\xBB\xBF{}"), 5));
...
TEST(JsonReaderTest, RejectsInvalidUtf8)
{
    ...
    EXPECT_TRUE(JsonReader::read(Utf8StringRef::from_data(fast, sizeof(fast) - 1)).is_err());
    // 慢路径（含转义 + 非法尾字节）：["\n<0xFF>"]
    const u8 slow[] = "[\"\\n\xFF\"]";
    EXPECT_TRUE(JsonReader::read(Utf8StringRef::from_data(slow, sizeof(slow) - 1)).is_err());
    // 对照：合法多字节内容不受影响
