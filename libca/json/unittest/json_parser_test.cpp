...
TEST(JsonParserSaxTest, EmitsEventsForArray)
{
    ...
    EXPECT_TRUE(p.parse());
    const std::vector<std::string> expected = {"[", "int:1", "str:x", "bool:true", "]"};
...
TEST(JsonParserSaxTest, EmitsEventsForObject)
{
    ...
    EXPECT_TRUE(p.parse());
    const std::vector<std::string> expected = {"{", "key:a", "int:1", "key:b", "null", "}"};
    EXPECT_EQ(env.h.events, expected);
}
...
TEST(JsonParserSaxTest, EmitsEventsForNested)
{
    ...
    EXPECT_TRUE(p.parse());
    const std::vector<std::string> expected = {"{", "key:arr", "[", "int:1", "int:2", "]", "}"};
    EXPECT_EQ(env.h.events, expected);
}
