    ...
    case JsonType::Object:
    {
        ...
        for (ca::usize i = 0; i < oa.size(); ++i) {
            ...
            if (oa[i].first != ob[i].first)
                return false;
            if (!equal(oa[i].second, ob[i].second))
                return false;
        }
        return true;
    }
...
TEST(JsonWriterTest, SerializesNaNAndInfinityAsNull)
{
    ...
    EXPECT_EQ(to_std(JsonWriter::write(
                  wrap(JsonValue::make_float(std::numeric_limits<ca::f64>::quiet_NaN())))),
              "null");
    EXPECT_EQ(to_std(JsonWriter::write(
                  wrap(JsonValue::make_float(std::numeric_limits<ca::f64>::infinity())))),
              "null");
    EXPECT_EQ(to_std(JsonWriter::write(
                  wrap(JsonValue::make_float(-std::numeric_limits<ca::f64>::infinity())))),
              "null");
...
TEST(JsonWriterTest, PreservesUtf8ByDefault)
{
    ...
    auto v =
        JsonValue::make_string(doc.arena().intern(Utf8String::from_cstr("\xE4\xB8\xAD")));   // "中"
...
TEST(JsonWriterTest, EscapesNonAsciiWhenOptionSet)
{
    ...
    JsonDocument doc;
    auto         v = JsonValue::make_string(
        doc.arena().intern(Utf8String::from_cstr("\xE4\xB8\xAD")));   // "中" = U+4E2D
...
TEST(JsonWriterTest, EscapesAstralPlaneAsSurrogatePair)
{
    ...
    JsonDocument doc;
    auto         v = JsonValue::make_string(
        doc.arena().intern(Utf8String::from_cstr("\xF0\x9F\x98\x80")));   // 😀 U+1F600
