...
std::string gen_flat_object(int n) {
    ...
    for (int i = 0; i < n; ++i) {
        if (i) s += ',';
        s += "\"key_"; s += std::to_string(i); s += "\":"; s += std::to_string(i);
    }
    s += "}";
    return s;
}
...
std::string gen_record_array(int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        s += ",\"name\":\"user_"; s += std::to_string(rng() % 100000);
        s += "\",\"email\":\"user"; s += std::to_string(i); s += "@example.com\"";
        s += ",\"active\":"; s += (i & 1) ? "true" : "false";
        s += ",\"score\":"; s += std::to_string((rng() % 10000) / 100.0);
        s += ",\"tags\":[\"a\",\"bb\",\"ccc\"]}";
    }
    s += "]";
    return s;
...
std::string gen_string_array(int n, int slen) {
    ...
    for (int i = 0; i < n; ++i) {
        if (i) s += ',';
        s += '"';
        for (int j = 0; j < slen; ++j) s += char('a' + (j % 26));
        s += '"';
    }
    s += "]";
    return s;
...
struct Res { double parse_ms; double write_ms; };
...
Res run_once(const std::string& text, bool do_write) {
    ...
    if (result.is_err()) { std::printf("  PARSE ERROR\n"); return r; }
...
void bench(const std::string& name, const std::string& text, int iters, bool do_write) {
    ...
    std::printf("%-28s | size=%7.2f MB | parse best=%8.3f ms (%6.1f MB/s)",
                name.c_str(), mb, best_parse, mb / (best_parse / 1000.0));
    if (do_write)
