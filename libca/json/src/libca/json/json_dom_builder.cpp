...
bool JsonDomBuilder::has_error() const noexcept
{
    return has_error_;
}

const ParseError& JsonDomBuilder::error() const noexcept
{
    return error_;
}
...
void JsonDomBuilder::emit_value(JsonValue v)
{
    ...
    if (top.is_array) {
        ...
        if (top.has_pending_key) {
            top.object_items.push_back(
                JsonValue::ObjectMember{std::move(top.pending_key), std::move(v)});
