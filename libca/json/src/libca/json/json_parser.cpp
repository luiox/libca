...
JsonParser::JsonParser(ca::str::Utf8StringRef input, JsonHandler& handler,
                       ca::str::Utf8StringArena& arena, const JsonParserOptions& options)
    : data_(input.data())
...
SourceLocation JsonParser::location() const noexcept
{
    return loc_;
}

const ParseError& JsonParser::last_error() const noexcept
{
    return error_;
}
...
void JsonParser::parse_literal(const char* text, ca::usize len, bool bool_value, const char* kind)
{
    SourceLocation start = loc_;
