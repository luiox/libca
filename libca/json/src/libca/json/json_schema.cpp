...
{
    return Utf8String(reinterpret_cast<const ca::u8*>(s.data()), static_cast<ca::usize>(s.size()));
}
    ...
    if (type_keyword.is_string()) {
        if (!match_one(type_keyword.as_string())) {
            record(
                ...
                schema_path,
                ca::str::format_std(
                    "expected {}, got {}", type_keyword.as_string(), type_name(instance.type())));
        }
        return Ok();
    }
...
void check_enum(const JsonValue& instance, const JsonValue& enum_keyword,
                ...
                std::vector<ValidationError>& errors)
{
    if (!enum_keyword.is_array())
        return;   // schema 形态校验在主流程
    for (const auto& candidate : enum_keyword.as_array()) {
        if (json_equal(instance, candidate))
            return;
    }
                          ...
                          std::vector<ValidationError>& errors)
{
    ...
    if (min != nullptr && min->is_number()) {
        ...
        double val =
            instance.is_int() ? static_cast<double>(instance.as_int()) : instance.as_float();
        if (val < limit) {
            record(errors, instance_path, join_path(schema_path, "minimum"), "value below minimum");
        }
    }
    ...
    if (max != nullptr && max->is_number()) {
        ...
        double val =
            instance.is_int() ? static_cast<double>(instance.as_int()) : instance.as_float();
        if (val > limit) {
            record(errors, instance_path, join_path(schema_path, "maximum"), "value above maximum");
        }
    }
}
                         ...
                         std::vector<ValidationError>& errors)
{
    ...
    if (min != nullptr && min->is_int() && cp_len < min->as_int()) {
        record(errors,
               instance_path,
               join_path(schema_path, "minLength"),
    ...
    if (max != nullptr && max->is_int() && cp_len > max->as_int()) {
        record(errors,
               instance_path,
               join_path(schema_path, "maxLength"),
...
bool validate_type_keyword_shape(const JsonValue& type_keyword, std::string& bad_value)
{
    auto known = [](const Utf8StringRef& s) -> bool {
        return s == Utf8StringRef::from_cstr("null") || s == Utf8StringRef::from_cstr("boolean") ||
               s == Utf8StringRef::from_cstr("integer") ||
               s == Utf8StringRef::from_cstr("number") || s == Utf8StringRef::from_cstr("string") ||
               s == Utf8StringRef::from_cstr("array") || s == Utf8StringRef::from_cstr("object");
    };
    if (type_keyword.is_string()) {
    ...
    if (type_keyword.is_array()) {
        for (const auto& entry : type_keyword.as_array()) {
            if (!entry.is_string() || !known(entry.as_string())) {
                bad_value =
                    entry.is_string() ? to_std(entry.as_string()) : std::string("<non-string>");
                return false;
            }
        }
        return true;
                                        ...
                                        std::vector<ValidationError>& errors)
{
    ...
    if (!schema.is_object()) {
        return Err(
            SchemaError{to_owning(std::string(schema_path)), owning("schema must be an object")});
    }
    ...
    if (const auto* t = schema.find(Utf8StringRef::from_cstr("type"))) {
        ...
        if (!validate_type_keyword_shape(*t, bad)) {
            return Err(
                SchemaError{to_owning(join_path(schema_path, "type")),
    ...
    if (instance.is_object()) {
        ...
        if (required != nullptr) {
            ...
            for (const auto& key : required->as_array()) {
                ...
                if (instance.find(key.as_string()) == nullptr) {
                    record(errors,
                           instance_path,
                           join_path(schema_path, "required"),
        ...
        if (props != nullptr) {
            ...
            for (const auto& [key, sub_schema] : props->as_object()) {
                ...
                if (field == nullptr)
                    continue;   // 缺失字段由 required 管，这里只校验存在的
                auto r = validate_node(*field,
                                       sub_schema,
                                       join_path(instance_path, to_std(key)),
                                       join_path(schema_path, to_std(key)),
                                       errors);
                if (r.is_err())
                    return r;
            }
    ...
    if (instance.is_array()) {
        ...
        if (items != nullptr) {
            ...
            for (ca::usize i = 0; i < arr.size(); ++i) {
                auto r = validate_node(arr[i],
                                       *items,
                                       join_path(instance_path, std::to_string(i)),
