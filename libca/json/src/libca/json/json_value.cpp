...
bool JsonValue::is_float() const noexcept
{
    return type_ == JsonType::Float;
}
bool JsonValue::is_number() const noexcept
{
    return type_ == JsonType::Int || type_ == JsonType::Float;
}
...
{
    if (type_ == JsonType::Float)
        return std::get<f64>(data_);
    if (type_ == JsonType::Int)
        return static_cast<f64>(std::get<i64>(data_));
    return fallback;
}
...
i64 JsonValue::as_int_or(i64 fallback) const noexcept
{
    if (type_ == JsonType::Int)
        return std::get<i64>(data_);
    if (type_ == JsonType::Float)
        return static_cast<i64>(std::get<f64>(data_));
    return fallback;
}
