...
{
public:
    Writer(const TomlWriterOptions& options)
        : options_(options)
    {}
    ...
    void emit(const char* s, ca::usize len) { sb_.append(s, len); }
    ...
    {
        if (!options_.indent_subtables)
            return;
        const ca::usize total = depth * options_.indent;
        for (ca::usize i = 0; i < total; ++i)
            sb_.append(" ");
    }
        ...
        case TomlDatetimeKind::OffsetDatetime:
        {
            ...
            if (dt.kind == TomlDatetimeKind::OffsetDatetime && dt.has_tz) {
                ...
                if (dt.tz_minutes == 0) {
                    ...
                    int     hh      = abs_min / 60;
                    int     mm      = abs_min % 60;
                    n += std::snprintf(buf + n, sizeof(buf) - n, "%c%02d:%02d", sign, hh, mm);
                }
            }
            break;
        }
    ...
    void write_basic_string(const u8* data, usize len)
    {
        ...
        for (usize i = 0; i < len; ++i) {
            ...
            default:
                if (c < 0x20 ||
                    c == 0x7F) {   // DEL 不在 TOML 1.0 未转义字符集，原样写出会被严格解析器拒绝
                    char buf[8];
...
void write_table(Writer& w, const TomlValue& tbl, const std::vector<ca::str::Utf8StringRef>& path)
{
    // 1) 先输出本表的"非子表"成员（标量/数组/inline table）为 key = value。
    bool wrote_kv = false;
    for (const auto& m : tbl.as_table()) {
        if (is_table_like(m.second))
            continue;
...
ca::Result<void, ca::str::Utf8String> TomlWriter::write_file(const ca::str::Utf8StringRef& path,
