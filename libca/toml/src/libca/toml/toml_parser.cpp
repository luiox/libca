...
const ParseError& TomlParser::last_error() const noexcept { return error_; }
...
bool TomlParser::at_end() const noexcept { return pos_ >= byte_length_; }
...
bool TomlParser::parse_line() {
    ...
    if (!parse_key_path(segments, seg_locs)) return false;
...
bool TomlParser::parse_key_path(std::vector<ca::str::Utf8StringRef>& out_segments,
    ...
    while (!failed_) {
        ...
        const u8 c = peek();
        if (c == '"') {
            if (!parse_basic_string(sb)) return false;
        } else if (c == '\'') {
            if (!parse_literal_string(sb)) return false;
...
bool TomlParser::parse_value(TomlValue& out) {
    if (failed_) return false;
    const u8 c = peek();
    if (c == 0) { fail(loc_, "unexpected end of input"); return false; }
    ...
    if (c == '"') {
        if (peek_at(1) == '"' && peek_at(2) == '"') {
            advance(); advance(); advance();  // """
            ca::str::Utf8StringBuilder sb;
            if (!parse_multiline_basic(sb)) return false;
    ...
    if (c == '\'') {
        if (peek_at(1) == '\'' && peek_at(2) == '\'') {
            advance(); advance(); advance();  // '''
            ca::str::Utf8StringBuilder sb;
            if (!parse_multiline_literal(sb)) return false;
            out = TomlValue::make_string(document_.arena().intern(sb.build_or_empty()));
            return !failed_;
        }
        return parse_string_value(out);
    ...
    if (c == 't' || c == 'f') {
        ...
        if (peek_at(0) == 'f' && peek_at(1) == 'a' && peek_at(2) == 'l' &&
            peek_at(3) == 's' && peek_at(4) == 'e') {
...
bool TomlParser::match_keyword_at(usize offset, const char* kw) const {
    ...
    if (pos_ + offset + kwlen > byte_length_) return false;
    for (usize i = 0; i < kwlen; ++i) {
        if (data_[pos_ + offset + i] != static_cast<u8>(kw[i])) return false;
    }
    return true;
}
...
bool TomlParser::parse_string_value(TomlValue& out) {
    ...
    const u8 c = peek();
    bool ok = false;
    if (c == '"') ok = parse_basic_string(sb);
    else if (c == '\'') ok = parse_literal_string(sb);
    else { fail(start, "expected string"); return false; }
    if (!ok) return false;
    out = TomlValue::make_string(document_.arena().intern(sb.build_or_empty()));
    return !failed_;
}
...
bool TomlParser::parse_basic_string(ca::str::Utf8StringBuilder& out) {
    ...
    advance();  // 开引号
    while (!failed_) {
        if (at_end()) { fail(start, "unterminated string"); return false; }
        const u8 c = peek();
        if (c == '"') { advance(); break; }
        if (c == '\n' || c == '\r') {
        ...
        if (c == '\\') {
            advance();
            if (at_end()) { fail(start, "unterminated escape"); return false; }
            const u8 esc = peek();
            advance();
            switch (esc) {
                ...
                case 'u': {
                    if (pos_ + 4 > byte_length_) { fail(start, "incomplete \\u escape"); return false; }
                    u32 cp = 0;
                    if (!parse_hex4(data_ + pos_, cp)) { fail(start, "invalid \\u escape"); return false; }
                    for (int i = 0; i < 4; ++i) advance();
                    if (cp >= 0xD800 && cp <= 0xDBFF) {
                        // 需要紧跟 \uXXXX 低代理
                        if (peek() == '\\' && peek_at(1) == 'u') {
                            advance(); advance();
                            if (pos_ + 4 > byte_length_) { fail(start, "incomplete low surrogate"); return false; }
                            u32 low = 0;
                            if (!parse_hex4(data_ + pos_, low)) { fail(start, "invalid low surrogate"); return false; }
                            for (int i = 0; i < 4; ++i) advance();
                            if (low < 0xDC00 || low > 0xDFFF) { fail(start, "expected low surrogate"); return false; }
                            u32 full = 0x10000 + ((cp - 0xD800) << 10) + (low - 0xDC00);
                            if (!out.append_code_point(full)) { fail(start, "invalid code point"); return false; }
                        } else {
                        ...
                        if (!out.append_code_point(cp)) { fail(start, "invalid code point"); return false; }
                    }
                    break;
                }
                case 'U': {
                    if (pos_ + 8 > byte_length_) { fail(start, "incomplete \\U escape"); return false; }
                    u32 cp = 0;
                    if (!parse_hex8(data_ + pos_, cp)) { fail(start, "invalid \\U escape"); return false; }
                    for (int i = 0; i < 8; ++i) advance();
                    if (cp >= 0xD800 && cp <= 0xDFFF) { fail(start, "surrogate in \\U escape"); return false; }
                    if (!out.append_code_point(cp)) { fail(start, "invalid code point"); return false; }
                    break;
                }
                default:
...
bool TomlParser::parse_multiline_basic(ca::str::Utf8StringBuilder& out) {
    ...
    while (!failed_) {
        if (at_end()) { fail(start, "unterminated multiline string"); return false; }
        const u8 c = peek();
        // 闭合 """
        if (c == '"' && peek_at(1) == '"' && peek_at(2) == '"') {
            ...
            if (peek_at(3) == '"') extra_quotes = 1;
            if (peek_at(3) == '"' && peek_at(4) == '"') extra_quotes = 2;
            for (usize k = 0; k < extra_quotes; ++k) {
                out.append("\"");
            }
            for (usize k = 0; k < 3 + extra_quotes; ++k) advance();
            break;
        }
        if (c == '\\') {
            ...
            while (lookahead < byte_length_) {
                ...
                if (lc == ' ' || lc == '\t' || lc == '\r') { ++lookahead; continue; }
                if (lc == '\n') { only_ws_to_eol = true; }
                break;
            }
            if (only_ws_to_eol) {
...
bool TomlParser::parse_multiline_literal(ca::str::Utf8StringBuilder& out) {
    ...
    while (!failed_) {
        if (at_end()) { fail(start, "unterminated multiline literal string"); return false; }
        const u8 c = peek();
        if (c == '\'' && peek_at(1) == '\'' && peek_at(2) == '\'') {
            usize extra_quotes = 0;
            if (peek_at(3) == '\'') extra_quotes = 1;
            if (peek_at(3) == '\'' && peek_at(4) == '\'') extra_quotes = 2;
            for (usize k = 0; k < extra_quotes; ++k) out.append("'");
            for (usize k = 0; k < 3 + extra_quotes; ++k) advance();
            break;
        }
        if (c == '\r') {
...
bool TomlParser::parse_array(TomlValue& out) {
    ...
    while (!failed_) {
        ...
        if (at_end()) { fail(start, "unterminated array"); break; }
        TomlValue elem;
...
bool TomlParser::parse_inline_table(TomlValue& out) {
    ...
    skip_inline_ws();
    if (failed_) { --depth_; return false; }
    if (peek() == '}') { advance(); --depth_; return true; }

    while (!failed_) {
        // 解析 dotted key
        std::vector<ca::str::Utf8StringRef> segments;
        std::vector<SourceLocation> seg_locs;
        if (!parse_key_path(segments, seg_locs)) { break; }
        skip_inline_ws();
        if (failed_) break;
        if (peek() != '=') { fail(loc_, "expected '=' in inline table"); break; }
        advance();
        skip_inline_ws();
        if (failed_) break;
        TomlValue val;
        ...
        if (failed_) break;
        const u8 c = peek();
        if (c == ',') { advance(); skip_inline_ws(); if (failed_) break; continue; }
        if (c == '}') { advance(); break; }
...
bool TomlParser::parse_number_or_datetime_value(TomlValue& out) {
    ...
    bool looks_like_date = remaining >= 10 &&
        is_digit(base[0]) && is_digit(base[1]) && is_digit(base[2]) && is_digit(base[3]) &&
        base[4] == '-' && is_digit(base[5]) && is_digit(base[6]) && base[7] == '-' &&
    ...
    while (scan < byte_length_) {
        ...
        if (c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '#' ||
            ...
            if (c == ' ' && looks_like_date && scan - pos_ == 10 &&
                scan + 1 < byte_length_ && scan + 9 <= byte_length_) {
                const u8* t = data_ + scan + 1;
                if (is_digit(t[0]) && is_digit(t[1]) && t[2] == ':' &&
                    is_digit(t[3]) && is_digit(t[4]) && t[5] == ':' &&
                    is_digit(t[6]) && is_digit(t[7])) {
                    ++scan;  // 吃掉空格，继续扫描时间部分
                    continue;
                }
            }
    ...
    if (try_parse_datetime(begin, len, dt, consumed_kind)) {
        ...
        switch (dt.kind) {
            case TomlDatetimeKind::OffsetDatetime:
                out = TomlValue::make_offset_datetime(dt); break;
            case TomlDatetimeKind::LocalDateTime:
                out = TomlValue::make_local_datetime(dt); break;
            case TomlDatetimeKind::LocalDate:
                out = TomlValue::make_local_date(dt); break;
            case TomlDatetimeKind::LocalTime:
                out = TomlValue::make_local_time(dt); break;
        }
        for (usize i = 0; i < consumed_kind; ++i) advance();
        return !failed_;
    }
...
bool TomlParser::try_parse_datetime(const u8* p, usize len, TomlDatetime& out, usize& consumed) {
    ...
    if (len >= 10 && is_digit(p[0]) && is_digit(p[1]) && is_digit(p[2]) && is_digit(p[3]) &&
        ...
        if (p[i] == 'T' || p[i] == 't' || p[i] == ' ') {
            ++i;
            // 期望时间 HH:MM:SS
            if (i + 8 > len) { consumed = 0; return false; }
            if (!(is_digit(p[i]) && is_digit(p[i+1]) && p[i+2] == ':' &&
                  is_digit(p[i+3]) && is_digit(p[i+4]) && p[i+5] == ':' &&
                  is_digit(p[i+6]) && is_digit(p[i+7]))) {
            ...
            if (hour > 23 || minute > 59 || second > 60) { consumed = 0; return false; }
            i += 8;
            ...
            if (i < len && p[i] == '.') {
                ...
                // 先数小数位数
                usize frac_end = i;
                while (frac_end < len && is_digit(p[frac_end])) ++frac_end;
                if (frac_end == frac_start) { consumed = 0; return false; }  // '.' 后无数字
            ...
            if (i < len) {
                if (p[i] == 'Z' || p[i] == 'z') {
                    ...
                    const u8 sign = p[i];
                    ++i;
                    if (i + 5 > len || !is_digit(p[i]) || !is_digit(p[i+1]) ||
                        p[i+2] != ':' || !is_digit(p[i+3]) || !is_digit(p[i+4])) {
                        consumed = 0; return false;
                    }
                    ca::u8 tz_h = static_cast<ca::u8>((p[i]-'0')*10 + (p[i+1]-'0'));
                    ca::u8 tz_m = static_cast<ca::u8>((p[i+3]-'0')*10 + (p[i+4]-'0'));
                    if (tz_h > 23 || tz_m > 59) { consumed = 0; return false; }
                    i += 5;
                    ca::i16 off = static_cast<ca::i16>(tz_h * 60 + tz_m);
                    if (sign == '-') off = static_cast<ca::i16>(-off);
                    out.has_tz = true;
                    out.tz_minutes = off;
                    out.kind = TomlDatetimeKind::OffsetDatetime;
                } else {
    ...
    // 仅时间：HH:MM:SS[.frac]
    if (len >= 8 && is_digit(p[0]) && is_digit(p[1]) && p[2] == ':' &&
        ...
        if (i < len && p[i] == '.') {
            ...
            usize frac_end = i;
            while (frac_end < len && is_digit(p[frac_end])) ++frac_end;
            if (frac_end == frac_start) { consumed = 0; return false; }
            usize ndigits = frac_end - frac_start;
            usize use = ndigits > 9 ? 9 : ndigits;
            ca::u32 frac = 0;
            for (usize k = 0; k < use; ++k) {
...
bool TomlParser::parse_number_decimal(const u8* begin, usize len, TomlValue& out) {
    ...
    if (len == 0) { fail(start, "invalid number"); return false; }

    // 前导零校验：整数部分（可选符号后）若首位是 '0'，则该位必须是整数部分的唯一数字。
    // 即 "0" 合法，"01" / "00.1" 非法。TOML 不允许前导零。
    {
        usize idx = 0;
        if (idx < len && (begin[idx] == '+' || begin[idx] == '-')) ++idx;
    ...
    if (buf.empty()) { fail(start, "invalid number"); return false; }
    buf.push_back('\0');
    ...
    if (is_float) {
        ...
        if (errno == ERANGE) { fail(start, "float out of range"); return false; }
        if (endp != buf.data() + buf.size() - 1) { fail(start, "invalid float"); return false; }
        out = TomlValue::make_float(d);
    } else {
        long long v = std::strtoll(buf.data(), &endp, 10);
        if (errno == ERANGE) { fail(start, "integer out of range (i64)"); return false; }
        if (endp != buf.data() + buf.size() - 1) { fail(start, "invalid integer"); return false; }
        out = TomlValue::make_integer(static_cast<ca::i64>(v));
    }
    // 推进游标
    for (usize i = 0; i < len; ++i) advance();
    return !failed_;
}
...
bool TomlParser::parse_integer_radix(const u8* begin, usize len, int radix, TomlValue& out) {
    ...
    for (usize i = 2; i < len; ++i) {
        const u8 c = begin[i];
        if (c == '_') {
            if (i == 2 || i + 1 == len) { fail(start, "invalid underscore in integer"); return false; }
            const u8 prev = begin[i-1];
            const u8 next = begin[i+1];
            // 进制整数下划线必须在两位数字之间
            bool ok = (prev >= '0' && prev <= '9') || (radix == 16 && ((prev|0x20) >= 'a' && (prev|0x20) <= 'f'));
            ok = ok && ((next >= '0' && next <= '9') || (radix == 16 && ((next|0x20) >= 'a' && (next|0x20) <= 'f')));
            if (!ok) { fail(start, "invalid underscore placement in integer"); return false; }
            continue;
        }
        buf.push_back(static_cast<char>(c));
    }
    ...
    if (errno == ERANGE) { fail(start, "integer out of range (i64)"); return false; }
    if (endp != buf.data() + buf.size() - 1) { fail(start, "invalid integer literal"); return false; }
    out = TomlValue::make_integer(static_cast<ca::i64>(v));
    for (usize i = 0; i < len; ++i) advance();
    return !failed_;
}
...
bool TomlParser::parse_special_float(const u8* begin, usize len, TomlValue& out) {
    ...
    usize i = 0;
    if (begin[0] == '+') { i = 1; }
    else if (begin[0] == '-') { i = 1; negative = true; }
    // 剩余应为 "inf" 或 "nan"
    usize rest = len - i;
    if (rest == 3) {
        if ((begin[i]|0x20) == 'i' && (begin[i+1]|0x20) == 'n' && (begin[i+2]|0x20) == 'f') {
            double v = negative ? -std::numeric_limits<double>::infinity()
                                :  std::numeric_limits<double>::infinity();
            out = TomlValue::make_float(v);
            for (usize k = 0; k < len; ++k) advance();
            return true;
        }
        if ((begin[i]|0x20) == 'n' && (begin[i+1]|0x20) == 'a' && (begin[i+2]|0x20) == 'n') {
