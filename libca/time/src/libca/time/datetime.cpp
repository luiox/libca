...
ca::core::Result<Date, std::string> Date::from_string(const std::string& date)
{
    ...
    const bool  shape_ok = is_ascii_digit(s[0]) && is_ascii_digit(s[1]) && is_ascii_digit(s[2]) &&
                          is_ascii_digit(s[3]) && s[4] == '-' && is_ascii_digit(s[5]) &&
                          is_ascii_digit(s[6]) && s[7] == '-' && is_ascii_digit(s[8]) &&
                          is_ascii_digit(s[9]);
    if (!shape_ok) {
        return ca::core::Err(std::string("Date: invalid format, expected YYYY-MM-DD"));
    }
    return ca::core::Ok(
        Date(digits_to_int(s, 4), digits_to_int(s + 5, 2), digits_to_int(s + 8, 2)));
}
...
ca::core::Result<Time, std::string> Time::from_string(const std::string& time)
{
    ...
    return ca::core::Ok(
        Time(digits_to_int(s, 2), digits_to_int(s + 3, 2), digits_to_int(s + 6, 2)));
}
