...
public:
    ...
    /// @brief 从微秒构造时间间隔。
    static constexpr Duration from_microseconds(ca::i64 value) noexcept
    {
        return Duration(value * 1000);
    }
    /// @brief 从毫秒构造时间间隔。
    static constexpr Duration from_milliseconds(ca::i64 value) noexcept
    {
        return Duration(value * 1000000);
    }
    /// @brief 从秒构造时间间隔。
    static constexpr Duration from_seconds(ca::i64 value) noexcept
    {
        return Duration(value * 1000000000);
    }
    /// @brief 从分钟构造时间间隔。
    static constexpr Duration from_minutes(ca::i64 value) noexcept
    {
        return from_seconds(value * 60);
    }
    /// @brief 从小时构造时间间隔。
    static constexpr Duration from_hours(ca::i64 value) noexcept
    {
    ...
    constexpr double  as_seconds_f64() const noexcept
    {
...
private:
    explicit constexpr Duration(ca::i64 nanoseconds) noexcept
