...
/// @note Timestamp 是纯 Unix epoch 纳秒值；除 now/is_past/is_future 外，转换与算术均可 constexpr
/// 求值。
class Timestamp
{
public:
    ...
    static constexpr Timestamp from_unix_nanoseconds(ca::i64 value) noexcept
    {
        return Timestamp(value);
    }
    /// @brief 从 Unix epoch 毫秒构造时间戳。
    static constexpr Timestamp from_unix_milliseconds(ca::i64 value) noexcept
    {
        return Timestamp(value * 1000000);
    }
    /// @brief 从 Unix epoch 秒构造时间戳。
    static constexpr Timestamp from_unix_seconds(ca::i64 value) noexcept
    {
        return Timestamp(value * 1000000000);
    }

    /// @brief 从 std::chrono::system_clock::time_point 构造时间戳。
    template<typename DurationT>
    static constexpr Timestamp
    from_time_point(std::chrono::time_point<std::chrono::system_clock, DurationT> value) noexcept
    {
...
private:
    explicit constexpr Timestamp(ca::i64 nanoseconds) noexcept
