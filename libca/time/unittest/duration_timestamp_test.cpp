...
static_assert(
    Timestamp::from_time_point(Timestamp::from_unix_seconds(2).to_time_point()).unix_seconds() == 2,
...
TEST(DurationTest, ChronoRoundtrip)
{
    ...
    EXPECT_EQ(std::chrono::duration_cast<std::chrono::milliseconds>(duration.to_chrono()).count(),
              1250);
}
