...
void Utf8StringPooledPtr::release() noexcept
{
    if (entry_ && --entry_->ref_count == 0) {
        ...
        if (entry_->owner)
            entry_->owner->erase_entry(entry_);
        else {
            delete[] entry_->data;
            delete entry_;
        }
...
void Utf8StringPool::erase_entry(Utf8PoolEntry* entry) noexcept
{
    ...
    if (it != hash_index_.end()) {
        ...
        if (bucket.empty())
            hash_index_.erase(it);   // 空桶删 key
    }
    if (active_count_)
        --active_count_;
    if (total_bytes_ >= entry->byte_length)
        total_bytes_ -= entry->byte_length;
    delete[] entry->data;
    delete entry;
}
...
Utf8StringPooledPtr Utf8StringPool::find(const Utf8StringRef& str) const
{
    ...
    if (it != hash_index_.end()) {
        for (auto* ep : it->second) {
            if (ep->byte_length == byte_length && std::memcmp(ep->data, data, byte_length) == 0) {
