...
WStringRef::WStringRef() noexcept
    : data_(nullptr)
    , length_(0)
{}
WStringRef::WStringRef(const wchar_t* data, usize length) noexcept
    : data_(data)
    , length_(length)
{}
WStringRef::WStringRef(const WString& str) noexcept
    : data_(str.data())
    , length_(str.length())
{}

usize WStringRef::length() const noexcept
{
    return length_;
}
bool WStringRef::is_empty() const noexcept
{
    return length_ == 0;
}
const wchar_t* WStringRef::data() const noexcept
{
    return data_;
}
wchar_t WStringRef::at(usize index) const
{
    return data_[index];
}
...
bool WStringRef::operator==(const WStringRef& other) const noexcept
{
    return equals(other);
}
bool WStringRef::operator!=(const WStringRef& other) const noexcept
...
WString::WString() noexcept
    : data_(new wchar_t[1]{0})
    , length_(0)
{}

WString::WString(const wchar_t* data, usize length)
    : data_(nullptr)
    , length_(0)
{
...
WString WString::from_wstr(const wchar_t* wstr)
{
    return WString(wstr);
}

usize WString::length() const noexcept
{
    return length_;
}
bool WString::is_empty() const noexcept
{
    return length_ == 0;
}
...
const wchar_t* WString::w_str() const noexcept
{
    return data_;
}
wchar_t WString::at(usize index) const
{
    return data_[index];
}

WStringRef WString::ref() const noexcept
{
    return WStringRef(data_, length_);
}
WStringRef WString::slice(usize start, usize end) const
{
    return ref().slice(start, end);
}
WString WString::substr(usize start, usize count) const
{
    return ref().substr(start, count);
}

int WString::compare(const WStringRef& other) const noexcept
{
    return ref().compare(other);
}
int WString::compare(const WString& other) const noexcept
{
    return ref().compare(other.ref());
}
bool WString::equals(const WStringRef& other) const noexcept
{
    return ref().equals(other);
}
bool WString::operator==(const WString& other) const noexcept
{
    return ref().equals(other.ref());
}
bool WString::operator==(const WStringRef& other) const noexcept
{
    return ref().equals(other);
}
bool WString::operator!=(const WString& other) const noexcept
...
{
    usize i = 0;
    while (i < length_ && iswspace(static_cast<wint_t>(data_[i])))
        ++i;
...
{
    usize i = length_;
    while (i > 0 && iswspace(static_cast<wint_t>(data_[i - 1])))
        --i;
    return slice(0, i);
}
...
bool WString::starts_with(const WStringRef& prefix) const noexcept
{
    return ref().starts_with(prefix);
}
bool WString::ends_with(const WStringRef& suffix) const noexcept
{
    return ref().ends_with(suffix);
}
WStringRef WString::trim() const noexcept
{
    return ref().trim();
}
WStringRef WString::trim_start() const noexcept
{
    return ref().trim_start();
}
WStringRef WString::trim_end() const noexcept
{
    return ref().trim_end();
}
std::vector<WStringRef> WString::split(const WStringRef& d) const
{
    return ref().split(d);
}
WString WString::to_lower() const
{
    return ref().to_lower();
}
WString WString::to_upper() const
{
    return ref().to_upper();
}
WString WString::replace_all(const WStringRef& from, const WStringRef& to) const
{
...
WStringBuilder::WStringBuilder() noexcept
    : buffer_(new wchar_t[kDefaultCapacity])
    , length_(0)
...
WStringBuilder& WStringBuilder::append(const WStringRef& str)
{
    return append(str.data(), str.length());
}
WStringBuilder& WStringBuilder::append(const WString& str)
{
    return append(str.data(), str.length());
}
...
WStringBuilder& WStringBuilder::append(wchar_t ch)
{
    return append(&ch, 1);
}
...
usize WStringBuilder::capacity() const noexcept
{
    return capacity_;
}
usize WStringBuilder::length() const noexcept
{
    return length_;
}
bool WStringBuilder::is_empty() const noexcept
{
    return length_ == 0;
}
void WStringBuilder::clear() noexcept
{
    length_ = 0;
}
WString WStringBuilder::build() const
{
    return WString(buffer_, length_);
}
