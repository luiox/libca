...
Utf8Char Utf8Char::from_raw(const u8* bytes) noexcept
{
    ...
    if ((b0 & 0xF0) == 0xE0) {
        if (bytes[1] == '\0' || bytes[2] == '\0')
            return Utf8Char(0);
        return Utf8Char(((u32)(b0 & 0x0F) << 12) | ((u32)(bytes[1] & 0x3F) << 6) |
                        ((u32)(bytes[2] & 0x3F)));
    }
    if ((b0 & 0xF8) == 0xF0) {
        if (bytes[1] == '\0' || bytes[2] == '\0' || bytes[3] == '\0')
            return Utf8Char(0);
        return Utf8Char(((u32)(b0 & 0x07) << 18) | ((u32)(bytes[1] & 0x3F) << 12) |
...
{
    // CJK 统一表意文字
    return (cp >= 0x4E00 && cp <= 0x9FFF) || (cp >= 0x3400 && cp <= 0x4DBF) ||   // CJK 扩展 A
...
static bool isLatinAlpha(u32 cp) noexcept
{
    return (cp >= 0x00C0 && cp <= 0x00D6) || (cp >= 0x00D8 && cp <= 0x00F6) ||
           (cp >= 0x00F8 && cp <= 0x00FF) || (cp >= 0x0100 && cp <= 0x017F) ||   // Latin Extended-A
...
bool Utf8Char::is_space() const noexcept
{
    ...
    // Unicode 空白字符
    if (cp_ == 0x00A0 || cp_ == 0x1680 || cp_ == 0x2000 || cp_ == 0x2001 || cp_ == 0x2002 ||
        cp_ == 0x2003 || cp_ == 0x2004 || cp_ == 0x2005 || cp_ == 0x2006 || cp_ == 0x2007 ||
        cp_ == 0x2008 || cp_ == 0x2009 || cp_ == 0x200A || cp_ == 0x2028 || cp_ == 0x2029 ||
        cp_ == 0x202F || cp_ == 0x205F || cp_ == 0x3000 || cp_ == 0xFEFF)
        return true;
    if (cp_ > 0xFFFF) {
        if constexpr (sizeof(wint_t) <= 2)
            return false;
