...
XmlNode::XmlNode() noexcept : type_(XmlNodeType::Text), data_(ca::str::Utf8StringRef{}) {}

XmlNode::XmlNode(XmlNode&& other) noexcept : type_(other.type_), data_(std::move(other.data_)) {
...
XmlNode XmlNode::clone() const {
    ...
    if (type_ == XmlNodeType::Element) {
        ...
        for (const auto& c : src.children) dup.children.push_back(c.clone());
...
XmlNodeType XmlNode::type() const noexcept { return type_; }

bool XmlNode::is_element() const noexcept { return type_ == XmlNodeType::Element; }
bool XmlNode::is_text() const noexcept    { return type_ == XmlNodeType::Text; }
bool XmlNode::is_comment() const noexcept { return type_ == XmlNodeType::Comment; }
bool XmlNode::is_cdata() const noexcept   { return type_ == XmlNodeType::Cdata; }
...
bool XmlNode::remove_attribute(const ca::str::Utf8StringRef& key) noexcept {
    ...
    auto& e = std::get<ElementData>(data_);
    auto it = e.attr_index.find(key_view(key));
    if (it == e.attr_index.end()) return false;
