...
{
    ca::usize& depth;
    explicit DepthGuard(ca::usize& d)
        : depth(d)
    {
        ++depth;
    }
...
const ParseError& XmlParser::last_error() const noexcept
{
    return error_;
}
...
u8 XmlParser::peek() const noexcept
{
...
bool XmlParser::at_end() const noexcept
{
...
bool XmlParser::is_ws(u8 c) noexcept
{
    return c == ' ' || c == '\t' || c == '\r' || c == '\n';
}
...
bool XmlParser::run()
{
    ...
    if (!parse_misc(document_.prolog()))
        return false;
    ...
    if (!parse_misc(document_.epilog()))
        return false;
    if (!at_end()) {
...
bool XmlParser::parse_misc(std::vector<XmlNode>& out)
{
    while (true) {
        ...
        if (at_end())
            return true;
        if (peek() != '<')
            return true;   // 元素起始或文本（顶层文本由调用方判定）
...
bool XmlParser::parse_attributes(XmlNode& element)
{
    while (true) {
        ...
        if (element.has_attribute(name)) {
            const std::string msg = ca::str::format_std("duplicate attribute '{}'", name);
...
bool XmlParser::parse_content(XmlNode& element)
{
    ...
    auto flush_text = [&]() -> void {
        ...
        const bool          drop =
            options_.trim_whitespace && all_whitespace(built.data(), built.byte_length());
        if (!drop) {
...
bool XmlParser::parse_reference(ca::str::Utf8StringBuilder& out)
{
    ...
    while (!at_end() && peek() != ';' && is_name_char(peek()))
        advance();
    if (at_end() || peek() != ';') {
        fail(amp_loc, "unterminated entity reference");
        return false;
    }
    const std::string_view ent(reinterpret_cast<const char*>(data_ + name_start),
                               pos_ - name_start);
    advance();   // ';'
    char decoded = 0;
    if (ent == "lt")
        ...
        const std::string msg = ca::str::format_std(
            "unknown entity reference '&{};' (custom entities/DTD not supported)", ent);
