...
{
public:
    explicit Writer(const XmlWriterOptions& options)
        : options_(options)
    {}
...
void emit_leaf_inline(Writer& w, const XmlNode& node)
{
    ...
    case XmlNodeType::Comment:
        ...
        w.emit("-->");
        break;
    case XmlNodeType::Element: break;   // 由 write_inline 处理
    }
}
...
ca::str::Utf8String XmlWriter::write(const XmlDocument& document, const XmlWriterOptions& options)
{
    ...
    for (const auto& node : document.epilog())
        write_block_child(w, node, 0);
    return w.build();
}
