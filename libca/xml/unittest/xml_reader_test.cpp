...
{
    return XmlReader::read(R(text)).is_err();
}
...
TEST(XmlReaderTest, UnclosedElement)
{
    EXPECT_TRUE(read_fails("<a><b></a>"));
}
...
TEST(XmlReaderTest, UnterminatedComment)
{
    EXPECT_TRUE(read_fails("<a><!-- oops</a>"));
}

TEST(XmlReaderTest, UnterminatedCdata)
{
    EXPECT_TRUE(read_fails("<a><![CDATA[oops</a>"));
}
...
TEST(XmlReaderTest, RejectsInvalidUtf8)
{
    ...
    EXPECT_TRUE(XmlReader::read(Utf8StringRef::from_data(bad_name, sizeof(bad_name) - 1)).is_err());
    // 文本节点含 GBK 字节（"张" 的 GBK 编码）
    const u8 bad_text[] = "<a>\xD5\xC5</a>";
    EXPECT_TRUE(XmlReader::read(Utf8StringRef::from_data(bad_text, sizeof(bad_text) - 1)).is_err());
    // 对照：合法 UTF-8 内容不受影响
