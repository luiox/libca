...
TEST(ResultTest, OrElseErr)
{
    ...
    auto                     recovered =
        r.or_else([](const std::string&) -> Result<int, std::string> { return Ok(42); });
...
TEST(ResultTest, OrElseOk)
{
    Result<int, std::string> r = Ok(10);
    auto                     recovered =
        r.or_else([](const std::string&) -> Result<int, std::string> { return Ok(99); });
    EXPECT_TRUE(recovered.is_ok());
...
TEST(ResultTest, OrElseChangeErrorType)
{
    ...
    auto recovered = r.or_else([](const std::string&) -> Result<int, bool> { return Err(true); });
...
TEST(ResultTest, AndThenOk)
{
    Result<int, std::string> r = Ok(5);
    auto                     next =
        r.and_then([](int x) -> Result<std::string, std::string> { return Ok(std::to_string(x)); });
...
{
    Result<int, std::string> r  = Ok(3);
    auto                     r2 = r.and_then([](int) -> Result<int, std::string> {
                   return Err("stop"s);
...
{
    Result<int, std::string> r = Ok(10);
    auto r2 = r.map([](int x) { return x * 2; }).and_then([](int x) -> Result<int, std::string> {
        return Ok(x + 3);
    });
