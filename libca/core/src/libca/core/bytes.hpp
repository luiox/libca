...
///       - **索引越界类**（slice/sub_slice 给了非法下标）：属调用方编程错误，仍抛
///       std::out_of_range。
...
public:
    ...
    /// 从指针+长度构造（不复制、不接管所有权）。
    ByteSlice(const u8* data, usize len) noexcept
        : data_(data)
        , len_(len)
    {}
...
inline usize Bytes::len() const noexcept
{
    return len_;
}
inline bool Bytes::is_empty() const noexcept
{
    return len_ == 0;
}
inline const u8* Bytes::as_ptr() const noexcept
{
    return ptr_ + pos_;
}
inline usize Bytes::remaining() const noexcept
{
    return len_ - pos_;
}
...
inline usize BytesMut::len() const noexcept
{
    return len_;
}
inline bool BytesMut::is_empty() const noexcept
{
    return len_ == 0;
}
inline const u8* BytesMut::as_ptr() const noexcept
{
    return data_.get() + pos_;
}
inline u8* BytesMut::as_mut_ptr() const noexcept
{
    return data_.get() + pos_;
}

inline usize BytesMut::remaining() const noexcept
{
    return len_ - pos_;
}
inline usize BytesMut::remaining_mut() const noexcept
{
    return capacity_ - len_;
}

inline void BytesMut::clear() noexcept
{
    len_ = 0;
    pos_ = 0;
}
...
inline bool BytesMut::operator==(const BytesMut& other) const noexcept
{
    return equals(other);
}
inline bool BytesMut::operator!=(const BytesMut& other) const noexcept
