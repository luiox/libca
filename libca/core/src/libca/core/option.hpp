...
class Option
{
    static_assert(!std::is_same<T, void>::value, "Option<void> is not allowed: use bool instead");
...
public:
    ...
    T unwrap_or(T default_value) const& { return value_ ? *value_ : std::move(default_value); }
    /// @copydoc unwrap_or()
    ...
    T unwrap_or_default() const& { return value_ ? *value_ : T{}; }
    /// @copydoc unwrap_or_default()
    T unwrap_or_default() && { return value_ ? std::move(*value_) : T{}; }
    ...
    auto map(Func map_fn) const& -> Option<std::decay_t<decltype(map_fn(std::declval<const T&>()))>>
    {
        ...
        if (value_)
            return Option<U>(std::in_place, map_fn(*value_));
        return None;
    }
    /// @copydoc map()
    template<typename Func>
    auto map(Func map_fn) && -> Option<std::decay_t<decltype(map_fn(std::declval<T&&>()))>>
    {
        ...
        if (value_)
            return Option<U>(std::in_place, map_fn(std::move(*value_)));
        return None;
    }
    ...
    bool operator==(const Option& other) const { return value_ == other.value_; }
...
template<typename U>
typename std::enable_if<!std::is_same<U, void>::value, Option<U>>::type Result<T, E>::ok() const&
{
...
template<typename T, typename E>
template<typename U>
typename std::enable_if<!std::is_same<U, void>::value, Option<U>>::type Result<T, E>::ok() &&
{
