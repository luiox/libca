    ...
    for (const char character : value) {
        if (character < '0' || character > '9')
            return ca::core::Err(HttpError::from_kind(HttpErrorKind::InvalidUrl,
                                                      "HTTP URL port contains a non-decimal byte"));
        port = port * 10 + static_cast<u32>(character - '0');
        if (port > std::numeric_limits<u16>::max())
            return ca::core::Err(
                HttpError::from_kind(HttpErrorKind::InvalidUrl, "HTTP URL port exceeds 65535"));
    }
    if (port == 0)
        return ca::core::Err(
...
HttpResult<HttpUrl> HttpUrl::parse(std::string_view value)
{
    ...
    if (scheme_end == std::string_view::npos)
        return ca::core::Err(HttpError::from_kind(HttpErrorKind::InvalidUrl,
                                                  "HTTP URL must contain an http or https scheme"));
    ...
    if (authority.front() == '[') {
        ...
        if (authority.find('[') != std::string_view::npos ||
            ...
            return ca::core::Err(HttpError::from_kind(HttpErrorKind::InvalidUrl,
                                                      "HTTP URL contains unmatched host brackets"));
        const auto colon = authority.rfind(':');
        if (colon != std::string_view::npos) {
            if (authority.find(':') != colon)
                return ca::core::Err(HttpError::from_kind(HttpErrorKind::InvalidUrl,
                                                          "HTTP URL IPv6 host must use brackets"));
    ...
    if (target.empty() || target.front() != '/')
        return ca::core::Err(HttpError::from_kind(HttpErrorKind::InvalidUrl,
                                                  "HTTP URL target must use origin-form"));

    return ca::core::Ok(HttpUrl(scheme, std::move(host), port, explicit_port, std::move(target)));
}
