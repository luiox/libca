...
YamlDocument read_ok(const char* text)
{
    ...
    if (result.is_err()) {
        ...
        ADD_FAILURE() << "parse failed: " << std::string(std::string_view(err.message)) << " (line "
                      << err.location.line << ", col " << err.location.column << ")";
        return YamlDocument();
    }
...
TEST(YamlReaderTest, RejectsInvalidUtf8)
{
    ...
    EXPECT_TRUE(
        YamlReader::read(Utf8StringRef::from_data(bad_plain, sizeof(bad_plain) - 1)).is_err());
    // 双引号标量里的非法尾字节。
    const u8 bad_quoted[] = "key: \"\xFF\"";
    EXPECT_TRUE(
        YamlReader::read(Utf8StringRef::from_data(bad_quoted, sizeof(bad_quoted) - 1)).is_err());
    // 对照：合法多字节内容不受影响。
