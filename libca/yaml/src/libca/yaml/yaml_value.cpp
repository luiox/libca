...
YamlValue::YamlValue() noexcept : type_(YamlType::Null), data_(std::monostate{}) {}
...
YamlType YamlValue::type() const noexcept { return type_; }

bool YamlValue::is_null() const noexcept     { return type_ == YamlType::Null; }
bool YamlValue::is_boolean() const noexcept  { return type_ == YamlType::Boolean; }
bool YamlValue::is_integer() const noexcept  { return type_ == YamlType::Integer; }
bool YamlValue::is_float() const noexcept    { return type_ == YamlType::Float; }
bool YamlValue::is_string() const noexcept   { return type_ == YamlType::String; }
bool YamlValue::is_sequence() const noexcept { return type_ == YamlType::Sequence; }
bool YamlValue::is_mapping() const noexcept  { return type_ == YamlType::Mapping; }
...
ca::f64 YamlValue::as_float_or(ca::f64 fallback) const noexcept {
    if (type_ == YamlType::Float)   return std::get<ca::f64>(data_);
    if (type_ == YamlType::Integer) return static_cast<ca::f64>(std::get<ca::i64>(data_));
    return fallback;
}

ca::i64 YamlValue::as_integer_or(ca::i64 fallback) const noexcept {
    if (type_ == YamlType::Integer) return std::get<ca::i64>(data_);
    if (type_ == YamlType::Float)   return static_cast<ca::i64>(std::get<ca::f64>(data_));
    return fallback;
}
...
bool YamlValue::remove(const ca::str::Utf8StringRef& key) noexcept {
    ...
    auto& map = std::get<MappingData>(data_);
    auto it = map.index.find(key_view(key));
    if (it == map.index.end()) return false;
