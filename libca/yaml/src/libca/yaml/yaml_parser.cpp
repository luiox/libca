...
{
    usize& depth;
    explicit DepthGuard(usize& d) noexcept
        : depth(d)
    {
        ++depth;
    }
...
const ParseError& YamlParser::last_error() const noexcept
{
    return error_;
}
...
bool YamlParser::advance_to_content()
{
    while (li_ < lines_.size()) {
        ...
        if (!L.blank && !L.comment_only) {
            usize c = col_;
            while (c < L.length && is_inline_ws(L.text[c]))
                ++c;
...
void YamlParser::skip_inline_ws() noexcept
{
    if (li_ >= lines_.size())
        return;
    const Line& L = lines_[li_];
    while (col_ < L.length && is_inline_ws(L.text[col_]))
        ++col_;
}
...
bool YamlParser::at_document_marker() const noexcept
{
    if (li_ >= lines_.size())
        return false;
    const Line& L = lines_[li_];
    if (L.indent != 0 || col_ != 0 || L.length < 3)
        return false;
    const u8 c = L.text[0];
    if (c != '-' && c != '.')
        return false;
    if (L.text[1] != c || L.text[2] != c)
        return false;
...
bool YamlParser::line_has_mapping_key() const noexcept
{
    ...
    if (first == '\'' || first == '"') {
        ...
        if (!closed)
            return false;   // 未闭合，交给标量路径报错
        while (i < L.length && is_inline_ws(L.text[i]))
            ++i;
    ...
    for (; i < L.length; ++i) {
        const u8 c = L.text[i];
        if (c == '#' && i > col_ && is_inline_ws(L.text[i - 1]))
            return false;
...
void YamlParser::parse_block_node(usize min_indent, YamlValue& out)
{
    ...
    if (reject_unsupported_indicator())
        return;
...
void YamlParser::parse_block_sequence(usize n, YamlValue& out)
{
    ...
    for (;;) {
        if (failed_)
            return;
        if (!advance_to_content() || cur_indent() < n)
            return;
        if (at_document_marker()) {
            fail(loc_here(), "multi-document YAML is not supported");
            return;
        ...
        if (!at_sequence_dash())
            return;   // 同缩进非 '-'：交还调用方（如父 mapping 的下一个 key）

        ++col_;   // 消费 '-'
        const Line& L     = lines_[li_];
        usize       after = col_;
        while (after < L.length && is_inline_ws(L.text[after]))
            ++after;
...
void YamlParser::parse_block_mapping(usize n, YamlValue& out)
{
    ...
    for (;;) {
        ...
        if (!parse_mapping_key(key, key_loc)) {
            if (!failed_)
                fail(key_loc, "expected ':' after mapping key");
            return;
        }
...
bool YamlParser::parse_mapping_key(ca::str::Utf8StringRef& out_key, SourceLocation& key_loc)
{
    ...
    for (; sep < L.length; ++sep) {
        ...
        if (c == '#' && sep > col_ && is_inline_ws(L.text[sep - 1]))
            break;
    ...
    usize key_end = sep;
    while (key_end > col_ && is_inline_ws(L.text[key_end - 1]))
        --key_end;
    if (key_end == col_) {
...
void YamlParser::parse_plain_scalar(YamlValue& out)
{
    ...
    for (; i < L.length; ++i) {
        const u8 c = L.text[i];
        if (c == '#' && i > start && is_inline_ws(L.text[i - 1]))
            break;   // 尾注释
        if (c == ':' && (i + 1 >= L.length || is_inline_ws(L.text[i + 1]))) {
            col_ = i;
    ...
    usize end = i;
    while (end > start && is_inline_ws(L.text[end - 1]))
        --end;
    ...
    while (col_ < L.length) {
        ...
        case 'x':
        {
            ...
            for (int k = 0; k < 2; ++k) {
                ...
                u8       d;
                if (h >= '0' && h <= '9')
                    d = static_cast<u8>(h - '0');
                else if (h >= 'a' && h <= 'f')
                    d = static_cast<u8>(h - 'a' + 10);
                else if (h >= 'A' && h <= 'F')
                    d = static_cast<u8>(h - 'A' + 10);
                else {
                    fail(start, "invalid \\x escape");
        ...
        default: fail(start, "invalid escape sequence in double-quoted string"); return false;
        }
    }
...
void YamlParser::parse_block_scalar(usize n, YamlValue& out)
{
    ...
    // 定基准缩进：首个非空白行（缩进须 > n）。
    usize first = li_;
    while (first < lines_.size() && lines_[first].blank)
        ++first;
    if (first >= lines_.size() || lines_[first].indent <= n) {
        // 空内容（chomp keep 的尾空行简化为空串，见设计文档）。
        out = YamlValue::make_string(
            document_.arena().intern(ca::str::Utf8StringBuilder().build_or_empty()));
        return;
    }
    ...
    while (li_ < lines_.size()) {
        ...
        for (usize idx : pending_blanks) {
            ...
            if (B.length > m)
                content.push_back({B.text + m, B.length - m});
            else
                content.push_back({nullptr, 0});
        }
    ...
    if (style == '|') {
        ...
        for (usize i = 0; i < content.size(); ++i) {
            if (i > 0)
                sb.append("\n", 1);
            if (content[i].first != nullptr)
                sb.append(content[i].first, content[i].second);
        }
    }
    else {
    ...
    if (chomp == 0) {
        ...
        sb.append("\n", 1);
        for (usize k = 0; k < trailing_blanks; ++k)
            sb.append("\n", 1);
    }
    // strip：不加
...
void YamlParser::parse_flow_value(YamlValue& out)
{
    ...
    const u8 c = L.text[col_];
    if (c == '[') {
        parse_flow_sequence(out);
        return;
    }
    if (c == '{') {
        parse_flow_mapping(out);
        return;
    }
    if (c == '\'') {
        ca::str::Utf8StringBuilder sb;
