...
{
public:
    explicit Writer(const YamlWriterOptions& options)
        : options_(options)
    {}
    ...
    static bool plain_is_safe(const u8* data, ca::usize len)
    {
        ...
        if (resolve_plain_scalar(data, len).kind != PlainScalarKind::String)
            return false;
        // 首尾空白。
        ...
        for (ca::usize i = 0; i < len; ++i) {
            ...
            if (is_control(c))
                return false;   // 含控制字符（\n 等）
            // ": " 或结尾 ':' → 会被当 mapping。
            if (c == ':' && (i + 1 >= len || data[i + 1] == ' ' || data[i + 1] == '\t'))
                return false;
            // " #" → 会被当注释。
...
void write_block_scalar_body(Writer& w, const u8* data, ca::usize len, ca::usize depth)
{
    ...
    for (i = 0; i <= effective; ++i) {
        if (i == effective || data[i] == '\n') {
            w.indent(depth);
            if (i > line_start)
                w.emit_bytes(data + line_start, i - line_start);
            w.newline();
            line_start = i + 1;
        }
    }
