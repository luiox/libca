...
bool is_octal_digit(u8 c) { return c >= '0' && c <= '7'; }
...
bool is_float_shape(const u8* data, usize len) {
    ...
    if (i < len && (data[i] == '+' || data[i] == '-')) ++i;
    // 整数部分或前导小数点
    while (i < len && is_digit(data[i])) { ++i; saw_digit = true; }
    if (i < len && data[i] == '.') {
        saw_dot = true;
        ++i;
        while (i < len && is_digit(data[i])) { ++i; saw_digit = true; }
    ...
    if (i < len && (data[i] == 'e' || data[i] == 'E')) {
        ...
        ++i;
        if (i < len && (data[i] == '+' || data[i] == '-')) ++i;
        if (i >= len || !is_digit(data[i])) return false;
        while (i < len && is_digit(data[i])) ++i;
    }
...
ResolvedScalar resolve_plain_scalar(const u8* data, usize len) noexcept {
    ...
    if (hex || oct) {
        ...
        for (usize i = 2; i < len; ++i) {
            if (hex ? !is_hex_digit(data[i]) : !is_octal_digit(data[i])) { ok = false; break; }
        }
        if (ok) {
