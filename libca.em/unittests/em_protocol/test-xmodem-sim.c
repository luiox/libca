...
static i32 mock_write(transport_t* self, const u8* buf, usize len)
{
    ...
    if (len > 0) {
        ...
        LOG_VERBOSE("dut->sim len=%u C=%u ACK=%u NAK=%u CAN=%u EOT=%u SOH=%u STX=%u",
                    ...
                    (unsigned int)count_soh,
                    (unsigned int)count_stx);
    }
    return len;
}
...
static void sim_init(sim_peer_t* sim, sim_role_t role, sim_proto_t proto, const u8* data, usize len)
{
    ...
    sim->seq          = 1;
    LOG_SUMMARY("sim init role=%u proto=%u len=%u",
                (unsigned int)role,
                (unsigned int)proto,
                (unsigned int)len);
}
...
static usize sim_gen_packet(sim_peer_t* sim, u8* out)
{
    ...
    if (sim->proto == SIM_PROTO_CHECKSUM) {
        u8 sum = 0;
        for (usize i = 0; i < packet_size; i++)
            sum += out[3 + i];
        out[idx++] = sum;
        LOG_VERBOSE("tx packet seq=%u offset=%u size=%u sum=0x%02X",
        ...
        out[idx++] = (crc >> 8) & 0xFF;
        out[idx++] = crc & 0xFF;
        LOG_VERBOSE("tx packet seq=%u offset=%u size=%u crc=0x%04X",
                    (unsigned int)sim->seq,
                    (unsigned int)sim->offset,
                    (unsigned int)packet_size,
                    (unsigned int)crc);
...
static usize sim_step(sim_peer_t* sim, const u8* in_buf, usize in_len, u8* out_buf)
{
    ...
    LOG_VERBOSE("sim step role=%u in_len=%u offset=%u seq=%u",
                (unsigned int)sim->role,
                (unsigned int)in_len,
                (unsigned int)sim->offset,
                (unsigned int)sim->seq);
                ...
                if (in_len >= total_len) {
                    ...
                    if ((u8)(seq + seq_inv) != 0xFF) {
                        ...
                        if (sim->rx_len + data_len <= sizeof(sim->rx_buffer)) {
                            ...
                            sim->rx_len += data_len;
                            LOG_SUMMARY("receiver packet ok seq=%u len=%u",
                                        (unsigned int)seq,
...
TEST_CASE(xmodem_rx_std_checksum)
{
    ...
    xmodem_config_t cfg = {.is_transmitter   = false,
...
TEST_CASE(xmodem_tx_crc)
{
    ...
    xmodem_config_t cfg = {.is_transmitter   = true,
                           .mode             = XMODEM_MODE_CRC,
                           .recv_buffer      = g_xm_internal_buf,
                           .recv_buffer_size = sizeof(g_xm_internal_buf),
                           .user_data        = NULL,
