...
TEST_CASE(ds_fixed_buf_read_helpers)
{
    ...
    TEST_ASSERT_EQUAL_UINT(
        0x3322, dstream_read_u16_le(&ds));   // reads 0x22,0x33 -> 0x3322 due to helper order
    TEST_ASSERT_EQUAL_UINT(0x44, dstream_read_u8(&ds));
    TEST_ASSERT_EQUAL_UINT(0x5566,
                           dstream_read_u16_be(&ds));   // reads 0x55,0x66 (big-endian -> 0x5566)
...
TEST_CASE(ds_fixed_buf_misc_helpers)
{
    ...
    dstream_reset(&ds, 2);
    TEST_ASSERT_EQUAL_UINT(
        4, dstream_available(&ds));   // capacity - used (cursor doesn't affect available)
}
