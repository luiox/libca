...
static inline uint8_t from5to8(uint8_t v)
{
    return (v << 3) | (v >> 2);
}
static inline uint8_t from6to8(uint8_t v)
{
    return (v << 2) | (v >> 4);
}
...
static int find_cache(const char* str, u8 size, u16 fg, u16 bg)
{
    for (int i = 0; i < TEXT_CACHE_SIZE; i++) {
        if (g_text_cache[i].tex && g_text_cache[i].fg == fg && g_text_cache[i].bg == bg &&
            g_text_cache[i].size == size && strcmp(g_text_cache[i].str, str) == 0) {
            return i;
        }
    }
    return -1;
...
static void create_cache_entry(int idx, const char* str, u8 size, u16 fg, u16 bg)
{
    ...
    SDL_Color fg_col      = {
        from5to8((fg >> 11) & 0x1F), from6to8((fg >> 5) & 0x3F), from5to8(fg & 0x1F), 255};
    SDL_Color bg_col = {
        from5to8((bg >> 11) & 0x1F), from6to8((bg >> 5) & 0x3F), from5to8(bg & 0x1F), 255};

    // 如果主字体大小和请求 size 不一致，尝试用 TTF_RenderUTF8_Shaded
    // 仍然有效（字体会被缩放），所以不额外打开文件
...
static void set_render_color_from565(u16 color)
{
    SDL_SetRenderDrawColor(g_renderer,
                           from5to8((color >> 11) & 0x1F),
                           from6to8((color >> 5) & 0x3F),
                           from5to8(color & 0x1F),
                           255);
}
...
{
    if (!g_renderer || !g_font || !g_display_on)
        return;
...
void st7735s_draw_string(u16 x, u16 y, u16 width, u16 height, u8 size, u8* p, u16 pen_color,
                         u16 back_color)
{
    if (!g_renderer || !p || !g_display_on)
        return;
...
static const st7735_ops_t g_ops = {.draw_string      = sdl_draw_string,
