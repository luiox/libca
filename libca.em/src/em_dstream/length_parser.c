...
static u32 calc_checksum(length_parser_t* self, const void* data, usize len, u32 prev)
{
    ...
    case LENGTH_PARSER_CKSUM_CRC16: return self->cksum_func.crc16(data, len, (u16)prev);
    case LENGTH_PARSER_CKSUM_CRC32: return self->cksum_func.crc32(data, len, prev);
    default: return 0;
    }
}
...
void length_parser_init(length_parser_t* self, dstream_t* ds, const u8* header, usize header_len,
                        u8 len_field_size, bool len_big_endian, u8 checksum_size,
                        bool checksum_big_endian, length_parser_cksum_type_t cksum_type,
                        length_parser_cksum_func_t cksum_func, u32 cksum_init_val,
                        usize max_frame_len)
{
    param_check(self != NULL);
    param_check(ds != NULL);
    param_check(len_field_size == 1 || len_field_size == 2 || len_field_size == 4);
    param_check(checksum_size == 0 || checksum_size == 1 || checksum_size == 2 ||
                checksum_size == 4);
    ...
    while (true) {
        ...
        case LENGTH_STATE_DATA_AND_CKSUM:
        {
            ...
            if (self->checksum_size > 0 && self->cksum_type != LENGTH_PARSER_CKSUM_NONE) {
                ...
                switch (self->cksum_type) {
                case LENGTH_PARSER_CKSUM_U8: checksum_ok = ((u8)calc_val == (u8)expect_val); break;
                case LENGTH_PARSER_CKSUM_CRC16:
                    checksum_ok = ((u16)calc_val == (u16)expect_val);
                    break;
                case LENGTH_PARSER_CKSUM_CRC32: checksum_ok = (calc_val == expect_val); break;
                default: checksum_ok = true; break;
                }
        ...
        default: length_parser_reset(self); return LENGTH_PARSER_ERR_SYNC;
        }
    }
}
...
void length_parser_consume(length_parser_t* self)
{
    ...
    usize total_len =
        self->header_len + self->len_field_size + self->target_len + self->checksum_size;
