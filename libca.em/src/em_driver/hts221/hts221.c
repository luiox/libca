...
#    define HTS221_I2C_WRITE(self, reg, buf, len) \
        port_hts221_i2c_write(                    \
            (self)->hi2c, (self)->dev_addr, (reg), 1, (u8*)(buf), (u16)(len), 0xFFFF)
#    define HTS221_I2C_READ(self, reg, buf, len) \
        port_hts221_i2c_read(                    \
            (self)->hi2c, (self)->dev_addr, (reg), 1, (u8*)(buf), (u16)(len), 0xFFFF)
...
#    define HTS221_I2C_WRITE(self, reg, buf, len) \
        g_hts221_port->i2c_write(                 \
            (self)->hi2c, (self)->dev_addr, (reg), 1, (u8*)(buf), (u16)(len), 0xFFFF)
#    define HTS221_I2C_READ(self, reg, buf, len) \
        g_hts221_port->i2c_read(                 \
            (self)->hi2c, (self)->dev_addr, (reg), 1, (u8*)(buf), (u16)(len), 0xFFFF)
...
{
    if (!self || !temperature10)
        return HTS221_ERR_INVALID_PARAM;
    ...
    // 1. 读取校准系数
    if (HTS221_I2C_READ(self, 0x32, &T0_degC_x8, 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x33, &T1_degC_x8, 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x35, &tmp, 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    ...
    if (HTS221_I2C_READ(self, 0x3C, &buffer[0], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x3D, &buffer[1], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x3E, &buffer[2], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x3F, &buffer[3], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    ...
    u32 timeout = 1000;   // ms
    while (timeout--) {
        if (HTS221_I2C_READ(self, HTS221_STATUS_REG, &status, 1) != 0)
            return HTS221_ERR_I2C_FAIL;
        if (status == 0x03)
            break;
    ...
    if (HTS221_I2C_READ(self, HTS221_TEMP_OUT_L, &buffer[0], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, HTS221_TEMP_OUT_H, &buffer[1], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
...
i32 hts221_read_humidity(hts221_t* self, int16_t* humidity10)
{
    ...
    // 1. 读取 H0_rH 和 H1_rH
    if (HTS221_I2C_READ(self, 0x30, &buf[0], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x31, &buf[1], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    H0_rh = (int16_t)(buf[0] >> 1);
    H1_rh = (int16_t)(buf[1] >> 1);

    // 2. 读取 H0_T0_OUT
    if (HTS221_I2C_READ(self, 0x36, &buf[0], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x37, &buf[1], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    H0_T0_out = (int16_t)(((u16)buf[1] << 8) | buf[0]);

    // 3. 读取 H1_T0_OUT
    if (HTS221_I2C_READ(self, 0x3A, &buf[0], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, 0x3B, &buf[1], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    ...
    if (HTS221_I2C_READ(self, HTS221_HUMIDITY_OUT_L, &buf[0], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
    if (HTS221_I2C_READ(self, HTS221_HUMIDITY_OUT_H, &buf[1], 1) != 0)
        return HTS221_ERR_I2C_FAIL;
