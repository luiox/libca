...
static i32 dht22_read_bits(dht22_t* self)
{
    ...
    if (wait > (DHT22_WAIT_PRESENCE_MAX_US / 2))
        return DHT22_ERR_NO_RESPONSE;

    // 检查 ACK 低脉冲
    wait = 0;
    while (!DHT_READ(self) && (wait++ < DHT22_ACK_MAX_US))
        DHT_DELAY_US(1);
    if ((wait < DHT22_ACK_MIN_US) || (wait > DHT22_ACK_MAX_US))
        return DHT22_ERR_BAD_ACK1;
    ...
    while (i < 40) {
        // 等待低电平开始
        wait = 0;
        while (!DHT_READ(self) && (wait++ < DHT22_BIT_WAIT_LOW_MAX_US))
            DHT_DELAY_US(1);
...
static i32 dht22_decode(u16* humidity10, i16* temperature10)
{
    ...
    if ((u8)(parity & 0xFF) != parity_rcv)
        return DHT22_ERR_CRC_FAIL;

    if (humidity10)
        *humidity10 = (u16)((hMSB << 8) | hLSB);
...
{
    if (!self)
        return DHT22_ERR_INVALID_PARAM;
