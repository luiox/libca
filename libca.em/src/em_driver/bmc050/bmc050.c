...
#    define BMC050_I2C_WRITE(hi2c, dev_addr, mem_addr, mem_addr_size, data, data_size, timeout) \
        g_bmc050_port->i2c_write(                                                               \
            (hi2c), (dev_addr), (mem_addr), (mem_addr_size), (data), (data_size), (timeout))
#    define BMC050_I2C_READ(hi2c, dev_addr, mem_addr, mem_addr_size, data, data_size, timeout) \
        g_bmc050_port->i2c_read(                                                               \
            (hi2c), (dev_addr), (mem_addr), (mem_addr_size), (data), (data_size), (timeout))
...
void bmc050_init(bmc050_t* self, void* hi2c, u16 dev_addr)
{
    ...
    bmc050_int_pin_config(
        self,
        BMC050_ACC_INT1_PP | BMC050_ACC_INT2_PP | BMC050_ACC_INT1_HIGH | BMC050_ACC_INT2_HIGH);
}

i32 bmc050_get_device_id(bmc050_t* self, u8* id)
{
    if (!self || !id)
        return BMC050_ERR_INVALID_PARAM;
...
i32 bmc050_set_bandwidth(bmc050_t* self, bmc050_acc_bw bw)
{
    ...
    u8  val = 0;
    i32 rc  = BMC050_I2C_READ(
        self->hi2c, self->dev_addr, BMC050_REG_ACC_FILTER, 1, &val, 1, I2C_TIMEOUT_DEFAULT);
    if (rc != 0) {
...
i32 bmc050_get_xyz(bmc050_t* self, int16_t* x, int16_t* y, int16_t* z)
{
    ...
    u8  buf[6] = {0};
    i32 rc     = BMC050_I2C_READ(
        self->hi2c, self->dev_addr, BMC050_REG_ACC_OUT_XL, 1, buf, 6, I2C_TIMEOUT_DEFAULT);
    if (rc != 0) {
...
i32 bmc050_get_irq_status(bmc050_t* self, bmc050_acc_irq* status)
{
    ...
    u8  buf[2] = {0};
    i32 rc     = BMC050_I2C_READ(
        self->hi2c, self->dev_addr, BMC050_REG_ACC_STATUS_IRQL, 1, buf, 2, I2C_TIMEOUT_DEFAULT);
    if (rc != 0)
        return BMC050_ERR_I2C_FAIL;
...
i32 bmc050_set_irq_mode(bmc050_t* self, bmc050_acc_im mode)
{
    ...
    if (mode == BMC050_ACC_IM_RESET) {
        u8  val = 0;
        i32 rc  = BMC050_I2C_READ(
            self->hi2c, self->dev_addr, BMC050_REG_ACC_IRQ_MODE, 1, &val, 1, I2C_TIMEOUT_DEFAULT);
        if (rc != 0)
            return BMC050_ERR_I2C_FAIL;
