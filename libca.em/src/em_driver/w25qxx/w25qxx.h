...
/// @param size 字节数
/// @param timeout 超时
extern void port_w25qxx_spi_transmit_receive(void* hspi, u8* tx_data, u8* rx_data, usize size,
                                             u32 timeout);
...
typedef struct w25qxx_port
{
    ...
    void (*spi_transmit_receive)(void* hspi, u8* tx_data, u8* rx_data, usize size,
                                 u32 timeout);   // SPI收发
} w25qxx_port_t;
