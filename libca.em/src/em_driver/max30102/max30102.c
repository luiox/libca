...
#    define MAX30102_I2C_WRITE(hi2c, dev, reg, reg_sz, data, sz, to) \
        port_max30102_i2c_write((hi2c), (dev), (reg), (reg_sz), (data), (sz), (to))
#    define MAX30102_I2C_READ(hi2c, dev, reg, reg_sz, data, sz, to) \
        port_max30102_i2c_read((hi2c), (dev), (reg), (reg_sz), (data), (sz), (to))

#elif (LIBCA_MAX30102_PORT_MODE == LIBCA_MAX30102_PORT_MODE_DYNAMIC)
static const max30102_port_t* g_max30102_port = NULL;
#    define MAX30102_I2C_WRITE(hi2c, dev, reg, reg_sz, data, sz, to) \
        g_max30102_port->i2c_write((hi2c), (dev), (reg), (reg_sz), (data), (sz), (to))
#    define MAX30102_I2C_READ(hi2c, dev, reg, reg_sz, data, sz, to) \
        g_max30102_port->i2c_read((hi2c), (dev), (reg), (reg_sz), (data), (sz), (to))
...
{
    g_max30102_port = port;
}
bool max30102_port_is_registered(void)
{
    return g_max30102_port != NULL;
}
#endif
                   ...
                   usize x_size, i32* y_buf, usize y_size)
{
    ...
    if (!max30102_write_reg(self, REG_INTR_ENABLE_1, 0xc0))
        return false;
    if (!max30102_write_reg(self, REG_INTR_ENABLE_2, 0x00))
        return false;
    if (!max30102_write_reg(self, REG_FIFO_WR_PTR, 0x00))
        return false;
    if (!max30102_write_reg(self, REG_OVF_COUNTER, 0x00))
        return false;
    if (!max30102_write_reg(self, REG_FIFO_RD_PTR, 0x00))
        return false;
    if (!max30102_write_reg(self, REG_FIFO_CONFIG, 0x0f))
        return false;
    if (!max30102_write_reg(self, REG_MODE_CONFIG, 0x03))
        return false;
    if (!max30102_write_reg(self, REG_SPO2_CONFIG, 0x27))
        return false;
    if (!max30102_write_reg(self, REG_LED1_PA, 0x24))
        return false;
    if (!max30102_write_reg(self, REG_LED2_PA, 0x24))
        return false;
    if (!max30102_write_reg(self, REG_PILOT_PA, 0x7f))
        return false;
...
bool max30102_read_fifo(max30102_t* self, u32* red_led, u32* ir_led)
{
    ...
    if (!max30102_read_reg(self, REG_INTR_STATUS_1, &status))
        return false;
    if (!max30102_read_reg(self, REG_INTR_STATUS_2, &status))
        return false;
...
static void find_peaks(i32* locs, i32* npks, i32* x, i32 size, i32 min_height, i32 min_distance,
                       i32 max_num)
{
...
void max30102_calculate(max30102_t* self, u32* ir_buffer, u32* red_buffer, max30102_data_t* result)
{
    ...
    if (result == NULL || ir_buffer == NULL || red_buffer == NULL || self == NULL)
        return;
    if (self->an_dx_buf == NULL || self->an_x_buf == NULL || self->an_y_buf == NULL)
        return;
    ...
    for (k = 0; k < MAX30102_BUFFER_SIZE; k++)
        self->an_x_buf[k] = (i32)ir_buffer[k] - (i32)ir_mean;

    for (k = 0; k < MAX30102_BUFFER_SIZE - MA4_SIZE; k++) {
        self->an_x_buf[k] = (self->an_x_buf[k] + self->an_x_buf[k + 1] + self->an_x_buf[k + 2] +
                             self->an_x_buf[k + 3]) /
                            4;
    }
    ...
    find_peaks(
        dx_peak_locs, &npks, self->an_dx_buf, MAX30102_BUFFER_SIZE - HAMMING_SIZE, th1, 8, 5);

    peak_interval_sum = 0;
    if (npks >= 2) {
        for (k = 1; k < npks; k++)
            peak_interval_sum += (dx_peak_locs[k] - dx_peak_locs[k - 1]);
    ...
    for (k = 0; k < MAX30102_BUFFER_SIZE - MA4_SIZE; k++) {
        self->an_x_buf[k] = (self->an_x_buf[k] + self->an_x_buf[k + 1] + self->an_x_buf[k + 2] +
                             self->an_x_buf[k + 3]) /
                            4;
        self->an_y_buf[k] = (self->an_y_buf[k] + self->an_y_buf[k + 1] + self->an_y_buf[k + 2] +
                             self->an_y_buf[k + 3]) /
                            4;
    }
    ...
    for (k = 0; k < exact_ir_valley_locs_count - 1; k++) {
        ...
        if (exact_ir_valley_locs[k + 1] - exact_ir_valley_locs[k] > 10) {
            for (i = exact_ir_valley_locs[k]; i < exact_ir_valley_locs[k + 1]; i++) {
                if (self->an_x_buf[i] > x_dc_max) {
                    x_dc_max     = self->an_x_buf[i];
                    x_dc_max_idx = i;
                }
                if (self->an_y_buf[i] > y_dc_max) {
                    y_dc_max     = self->an_y_buf[i];
                    y_dc_max_idx = i;
                }
            }
            y_ac = (self->an_y_buf[exact_ir_valley_locs[k + 1]] -
                    self->an_y_buf[exact_ir_valley_locs[k]]) *
                   (y_dc_max_idx - exact_ir_valley_locs[k]);
            y_ac = self->an_y_buf[exact_ir_valley_locs[k]] +
                   y_ac / (exact_ir_valley_locs[k + 1] - exact_ir_valley_locs[k]);
            y_ac = self->an_y_buf[y_dc_max_idx] - y_ac;

            x_ac = (self->an_x_buf[exact_ir_valley_locs[k + 1]] -
                    self->an_x_buf[exact_ir_valley_locs[k]]) *
                   (x_dc_max_idx - exact_ir_valley_locs[k]);
            x_ac = self->an_x_buf[exact_ir_valley_locs[k]] +
                   x_ac / (exact_ir_valley_locs[k + 1] - exact_ir_valley_locs[k]);
    ...
    if (middle_idx > 1)
        ratio_average = (ratio[middle_idx - 1] + ratio[middle_idx]) / 2;
    else
