...
#    define BMP180_I2C_WRITE(hi2c, dev_addr, mem_addr, mem_addr_size, data, data_size, timeout) \
        g_bmp180_port->i2c_write(                                                               \
            (hi2c), (dev_addr), (mem_addr), (mem_addr_size), (data), (data_size), (timeout))
#    define BMP180_I2C_READ(hi2c, dev_addr, mem_addr, mem_addr_size, data, data_size, timeout) \
        g_bmp180_port->i2c_read(                                                               \
            (hi2c), (dev_addr), (mem_addr), (mem_addr_size), (data), (data_size), (timeout))
...
{
    if (!self)
        return BMP180_ERR_INVALID_PARAM;
    ...
    /* 读取校准参数 (22 字节) */
    u8 buf[22];
    if (bmp180_read_regs(self, BMP180_REG_CALIB, buf, 22) != 0)
        return BMP180_ERR_I2C_FAIL;
...
{
    u8 id = 0;
    if (bmp180_read_reg(self, BMP180_REG_ID, &id) != 0)
        return BMP180_ERR_I2C_FAIL;
...
{
    if (bmp180_write_reg(self, BMP180_REG_CTRL, BMP180_CMD_TEMP) != 0)
        return BMP180_ERR_I2C_FAIL;
    BMP180_DELAY_MS(5);   // Wait 4.5ms
    u8 buf[2];
    if (bmp180_read_regs(self, BMP180_REG_DATA, buf, 2) != 0)
        return BMP180_ERR_I2C_FAIL;
    if (ut)
        *ut = (u16)((buf[0] << 8) | buf[1]);
    return BMP180_OK;
}
...
i32 bmp180_read_raw_press(bmp180_t* self, bmp180_oss_t oss, u32* up)
{
    ...
    if (bmp180_write_reg(self, BMP180_REG_CTRL, cmd) != 0)
        return BMP180_ERR_I2C_FAIL;

    u32 wait_ms = 5;
    if (oss == BMP180_OSS_STANDARD)
        wait_ms = 8;
    else if (oss == BMP180_OSS_HIGH_RES)
        wait_ms = 14;
    else if (oss == BMP180_OSS_ULTRA_RES)
        wait_ms = 26;
    BMP180_DELAY_MS(wait_ms);

    u8 buf[3];
    if (bmp180_read_regs(self, BMP180_REG_DATA, buf, 3) != 0)
        return BMP180_ERR_I2C_FAIL;
    if (up)
        *up = (u32)(((u32)buf[0] << 16) | ((u32)buf[1] << 8) | (u32)buf[2]) >> (8 - oss);
    return BMP180_OK;
}
...
i32 bmp180_read_temp(bmp180_t* self, f32* temperature)
{
    ...
    if (temperature)
        *temperature = (f32)((self->calib.B5 + 8) >> 4) / 10.0f;
    return BMP180_OK;
}
...
i32 bmp180_read_press(bmp180_t* self, bmp180_oss_t oss, i32* pressure)
{
    ...
    if (pressure)
        *pressure = bmp180_calc_pressure(self, up, oss);
    return BMP180_OK;
}
