...
#    define TOFXXF_UART_RECV(huart, buf, len, timeout_ms) \
        port_tofxxf_uart_recv((huart), (buf), (len), (timeout_ms))
...
#    define TOFXXF_UART_RECV(huart, buf, len, timeout_ms) \
        g_tofxxf_port->uart_recv((huart), (buf), (len), (timeout_ms))
...
static usize build_request_frame(u8 slave_addr, u8 func, const u8* data, usize data_len,
...
static i32 validate_response(const u8* frame, usize len, u8 expected_addr, u8 expected_func)
{
    if (len < 5) {
...
i32 tofxxf_read_reg(tofxxf_t* self, u16 reg_addr, u16 count, u8* out_buf, usize out_buf_size)
{
    param_check(self != NULL);
    ...
    // 构建请求帧
    u8    req_frame[8];
    usize req_len = build_request_frame(
        self->slave_addr, TOFXF_FUNC_READ_HOLDING, req_data, 4, req_frame, sizeof(req_frame));
    if (req_len == 0) {
    ...
    // 验证响应
    i32 err = validate_response(resp_frame, recv_len, self->slave_addr, TOFXF_FUNC_READ_HOLDING);
    if (err != TOFXF_OK) {
...
i32 tofxxf_write_reg(tofxxf_t* self, u16 reg_addr, u16 value)
{
    ...
    u8    req_frame[8];
    usize req_len = build_request_frame(
        self->slave_addr, TOFXF_FUNC_WRITE_SINGLE, req_data, 4, req_frame, sizeof(req_frame));
    if (req_len == 0) {
        return TOFXF_ERR_SEND;
    }
    ...
    // 验证响应
    i32 err = validate_response(resp_frame, recv_len, self->slave_addr, TOFXF_FUNC_WRITE_SINGLE);
    if (err != TOFXF_OK) {
        debug_print("[tofxxf] response validation failed, err:%d\n", err);
        return err;
    }
