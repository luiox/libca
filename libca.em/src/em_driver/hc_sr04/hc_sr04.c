...
#    define HC_WRITE_PIN(self, v) \
        g_hc_sr04_port->write_pin((self)->trig_port, (self)->trig_pin, (v))
...
#    define HC_MUTEX_PEND()                   \
        do {                                  \
            if (g_hc_sr04_port->mutex_pend)   \
                g_hc_sr04_port->mutex_pend(); \
        } while (0)
#    define HC_MUTEX_POST()                   \
        do {                                  \
            if (g_hc_sr04_port->mutex_post)   \
                g_hc_sr04_port->mutex_post(); \
        } while (0)
