...
static uint8_t ICM20948_Init(void)
{
    ...
    writebuf =
        0x04;   // 设置GYRO的采样频率为1.1KHz（需要FCHOICE=1(启用滤波功能),DLPF_CFG=1~7(配置滤波的信息)）
    check_state += iicdev->write_reg(ICM20948_DEV << 1, GYRO_SMPLRT_DIV, &writebuf, 1, 100);

    // 配置 GYRO_CONFIG_1，配置DLPCFG，配置量程，配置DLFP
    // FS_SEL[2:1]陀螺仪量程配置：（3<<1：正负2000dps 2<<1:正负1000dps 1<<1:正负500dps
    // 0<<1:正负250dps） FCHOICE[0]滤波使能 ： 1 --> 开启滤波 DLPFCFG[5:3]滤波情况配置：5 （3DB
    // BW:11.6Hz NBW:17.8Hz）3DB BW:频率越高，陀螺仪响应速度越快，但噪点可能增加
    // NBW:频率越低，数据越稳定
...
static void ImuUpdate(IMU_DATA_t* data)
{
    ...
    // 获得3轴磁力计数据
    uint8_t magnbuf[8];   // byte[6]作废,地址0x17无数据。byte[7]为寄存器ST2,地址0x18.
                          // 为了加快效率使用连续方式读取
...
static void attitudeUpdate(IMU_DATA_t imudata,
                           ATTITUDE_DATA_t* attitude)   // 显示互补滤波（陀螺仪 + 加速度计）
{
...
// 驱动挂载
IMUInterface_t UserICM20948 = {.Init                     = ICM20948_Init,
