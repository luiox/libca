...
#    define BMP280_I2C_WRITE(hi2c, dev_addr, mem_addr, mem_addr_size, data, data_size, timeout) \
        g_bmp280_port->i2c_write(                                                               \
            (hi2c), (dev_addr), (mem_addr), (mem_addr_size), (data), (data_size), (timeout))
#    define BMP280_I2C_READ(hi2c, dev_addr, mem_addr, mem_addr_size, data, data_size, timeout) \
        g_bmp280_port->i2c_read(                                                               \
            (hi2c), (dev_addr), (mem_addr), (mem_addr_size), (data), (data_size), (timeout))
...
static f32 bmp280_compensate_T(bmp280_t* self, i32 ut)
{
    ...
    v_x2 = (((((ut >> 4) - ((i32)self->calib.dig_T1)) * ((ut >> 4) - ((i32)self->calib.dig_T1))) >>
             12) *
...
static f32 bmp280_compensate_P(bmp280_t* self, i32 up)
{
    ...
    v1 = (((f32)self->calib.dig_P3) * v1 * v1 / 524288.0f + ((f32)self->calib.dig_P2) * v1) /
         524288.0f;
...
{
    if (!self)
        return BMP280_ERR_INVALID_PARAM;
    ...
    /* 读取校准参数 (24 字节) */
    u8 buf[24];
    if (bmp280_read_regs(self, BMP280_REG_CALIB, buf, 24) != 0)
        return BMP280_ERR_I2C_FAIL;
...
{
    u8 id = 0;
    if (bmp280_read_reg(self, BMP280_REG_ID, &id) != 0)
        return BMP280_ERR_I2C_FAIL;
                  ...
                  bmp280_filter_t filter, bmp280_standby_t standby)
{
    ...
    if (bmp280_write_reg(self, BMP280_REG_CTRL_MEAS, ctrl_meas) != 0)
        return BMP280_ERR_I2C_FAIL;
    if (bmp280_write_reg(self, BMP280_REG_CONFIG, config) != 0)
        return BMP280_ERR_I2C_FAIL;
...
{
    u8 ctrl_meas;
    if (bmp280_read_reg(self, BMP280_REG_CTRL_MEAS, &ctrl_meas) != 0)
        return BMP280_ERR_I2C_FAIL;
...
{
    u8 status;
    if (bmp280_read_reg(self, BMP280_REG_STATUS, &status) != 0)
        return false;
...
{
    u8 buf[3];
    if (bmp280_read_regs(self, BMP280_REG_TEMP_MSB, buf, 3) != 0)
        return BMP280_ERR_I2C_FAIL;
...
i32 bmp280_read_press(bmp280_t* self, f32* pressure)
{
    ...
    u8 buf[3];
    if (bmp280_read_regs(self, BMP280_REG_PRESS_MSB, buf, 3) != 0)
        return BMP280_ERR_I2C_FAIL;
...
{
    u8 buf[6];
    if (bmp280_read_regs(self, BMP280_REG_PRESS_MSB, buf, 6) != 0)
        return BMP280_ERR_I2C_FAIL;
