...
// 遍历链表（使用 for 循环）
#define slist_for_each(node, head) for (slist_node_t* node = (head)->next; node; node = node->next)

// 安全遍历，允许在遍历时删除当前节点（使用临时变量 next）
#define slist_for_each_safe(node, head, next_node)                                           \
    for (slist_node_t* node = (head)->next, *next_node; node && (next_node = node->next, 1); \
         node               = next_node)
