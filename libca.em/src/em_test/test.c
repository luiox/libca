                       ...
                       uint8_t expected, uint8_t actual)
{
    if (expected != actual) {
        test_output(
            "  ✗ %s:%d: EXPECT_EQ_U8(%s, %s) failed: expected %u (0x%02X), got %u (0x%02X)\n",
            file,
            line,
            expr_e,
            expr_a,
                       ...
                       int8_t expected, int8_t actual)
{
    if (expected != actual) {
        test_output(
            "  ✗ %s:%d: EXPECT_EQ_I8(%s, %s) failed: expected %d (0x%02X), got %d (0x%02X)\n",
            file,
            line,
            expr_e,
            expr_a,
                        ...
                        uint16_t expected, uint16_t actual)
{
    if (expected != actual) {
        test_output(
            "  ✗ %s:%d: EXPECT_EQ_U16(%s, %s) failed: expected %u (0x%04X), got %u (0x%04X)\n",
            file,
            line,
            expr_e,
            expr_a,
                        ...
                        int16_t expected, int16_t actual)
{
    if (expected != actual) {
        test_output(
            "  ✗ %s:%d: EXPECT_EQ_I16(%s, %s) failed: expected %d (0x%04X), got %d (0x%04X)\n",
            file,
            line,
            expr_e,
            expr_a,
                        ...
                        uint32_t expected, uint32_t actual)
{
    if (expected != actual) {
        test_output(
            "  ✗ %s:%d: EXPECT_EQ_U32(%s, %s) failed: expected %lu (0x%08lX), got %lu (0x%08lX)\n",
            file,
            line,
            expr_e,
            expr_a,
                        ...
                        int32_t expected, int32_t actual)
{
    if (expected != actual) {
        test_output(
            "  ✗ %s:%d: EXPECT_EQ_I32(%s, %s) failed: expected %ld (0x%08lX), got %ld (0x%08lX)\n",
            file,
            line,
            expr_e,
            expr_a,
                        ...
                        float expected, float actual, float epsilon)
{
    ...
    if (diff > epsilon) {
        test_output("  ✗ %s:%d: EXPECT_EQ_F32(%s, %s) failed: expected %f, got %f (diff: %f, "
                    "epsilon: %f)\n",
                    file,
                    line,
                    expr_e,
                        ...
                        double expected, double actual, double epsilon)
{
    ...
    if (diff > epsilon) {
        test_output("  ✗ %s:%d: EXPECT_EQ_F64(%s, %s) failed: expected %f, got %f (diff: %f, "
                    "epsilon: %f)\n",
                    file,
                    line,
                    expr_e,
...
void test_assert_null(const char* file, int line, const char* expr, const void* ptr)
{
    if (ptr != NULL) {
        test_output(
            "  ✗ %s:%d: EXPECT_NULL(%s) failed: pointer is not NULL (%p)\n", file, line, expr, ptr);
        g_assert_failed++;
        current_test_failed = 1;
    }
    else {
