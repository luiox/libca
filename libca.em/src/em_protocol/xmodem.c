        ...
        if (acumulate_timer_get_elapsed(&xm->retry_timer) >= 3000) {
            if (xm->retry_count < xm->config->max_retries || xm->config->max_retries == -1) {
                ...
                if ((xm->config->mode == XMODEM_MODE_CRC || xm->config->mode == XMODEM_MODE_1K) &&
                    ...
                    // Reset retry timer to send NAK immediately? No, wait next tick cycle
                    // effectively. But maybe we should send immediately? The timer just triggered
                    // so we are about to send.
                }

                // 根据模式发送启动字符
                // 标准模式发送NAK，CRC模式发送'C'
                u8 start_char =
                    (xm->current_mode == XMODEM_MODE_STANDARD) ? XMODEM_NAK : XMODEM_CRC;
...
static void xmodem_send_data_packet(xmodem_t* xm)
{
    ...
    i32 read_len = xm->cbs->on_send(xm->config->user_data, xm->offset, &packet_buf[3], data_size);
    // printf("Read len: %d\n", read_len);
            ...
            if (xm->received_len >= packet_total) {
                ...
                if (seq == (u8)(~nseq) && check_ok &&
                    ...
                    if (seq == xm->packet_num) {
                        // 新包
                        if (xm->cbs->on_recv(xm->config->user_data, xm->offset, data, data_len) ==
                            0) {
                            xm->offset += data_len;
                            xm->packet_num++;
                            xmodem_send_char(xm, XMODEM_ACK);
                    ...
                    // 校验失败或序号不对
                    xm->retry_count++;
                    if (xm->config->max_retries != -1 &&
                        xm->retry_count > xm->config->max_retries) {
                        xmodem_send_char(xm, XMODEM_CAN);
                        xm->state = S_ERROR;
                        xm->cbs->on_finish(xm->config->user_data, XMODEM_ERR_RETRY_EXCEED);
                    }
