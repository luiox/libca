...
em_driver 的驱动通常不编写单元测试，因为驱动与硬件耦合紧密，许多行为无法在纯软件环境中复现或断言。**但要求在驱动文档中提供明确的硬件验证计划**（测试步骤、所需设备与夹具、测试向量和验收标准）。对于驱动内部的纯逻辑部分（例如数据解码、校验或状态机），建议将其抽离到无硬件依赖的模块并为该模块编写单元测试（遵循 `prompt/em_code_rule.md` 的单元测试规范）。同时，在 PR 中应附上集成或硬件测试的复现步骤与测试结果（日志或截图）。
