...
em_driver 模块通常不编写单元测试，因为它们与硬件耦合紧密，很多行为难以在纯软件环境中复现。**要求**：在驱动文档中提供硬件验证计划（包含测试步骤、所需设备/夹具、测试数据与验收标准）。对于驱动中可拆分的无硬件依赖逻辑，建议将其抽离到独立模块并为该模块编写单元测试（遵循 `prompt/em_code_rule.md` 的单元测试规范，使用 `TEST_ENABLE` 与 `em_test` 的 mock 能力）。参考模板：`.codebuddy/skills/em-driver-dev/assets/driver_testing_plan.md`。取而代之，可以通过以下方式进行测试：
