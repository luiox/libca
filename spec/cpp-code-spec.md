...
- **公开 API 的使用文档写在头文件里**，统一使用 Doxygen 行注释 `/// `。公开 API 包括类、结构体、枚举、函数、方法、公开常量和公开宏等；按需使用 `@brief`、`@param`、`@return` 和 `@note`。头文件是「这个函数怎么用」的唯一事实来源——调用方查头文件即得 API。
- 非 API 注释（包括文件说明、章节分隔、实现细节、测试说明和内部函数说明）使用普通 `//` 或 `/* */`，禁止使用 `///`。已在头文件注释的公开 API 不得在 `.cpp` 定义处重复注释。
