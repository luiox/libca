...
namespace libca
{
    ConnectionPool::ConnectionPool()
    {
        ...
        std::thread productThread(
          std::bind(&ConnectionPool::productConnectionTask, this));
        productThread.detach(); // 分离线程

        // 启动一个线程扫描连接池中的连接，如果连接空闲时间超过最大空闲时间m_maxIdleTime，那么就关闭连接
        std::thread scannerThread(
          std::bind(&ConnectionPool::scannerConnectionTask, this));
        scannerThread.detach(); // 分离线程
    }

    bool
    ConnectionPool::loadFromConfigFile(const std::string & filename)
    {
        // 从ini文件加载配置
        return true;
    }
    ...
    void
    ConnectionPool::productConnectionTask()
    {
        for (;;) {
    ...
    // 扫描连接的任务
    void
    ConnectionPool::scannerConnectionTask()
    {
        for (;;) {
            // 模拟定时睡眠效果
