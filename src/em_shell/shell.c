...
// 全局当前 simple shell 对象（只允许一个）
static simple_shell_t *g_simple_shell_current = NULL;
// 命令表与计数
static simple_shell_cmd_t g_simple_shell_cmds[SIMPLE_SHELL_MAX_CMD];
static i32 g_simple_shell_cmd_count = 0;

// 前置声明：内置命令
static i32 simple_shell_help(i32 argc, char *argv[]);
static i32 simple_shell_clear(i32 argc, char *argv[]);

// 初始化 simple shell 并注册内置命令
void simple_shell_init(simple_shell_t *shell, char *buffer, u16 size)
{
    shell->buffer = buffer;
    shell->buffer_size = size;
    if (g_simple_shell_current == NULL) g_simple_shell_current = shell;

    /* 注册内置命令 */
    simple_shell_register_command("help", simple_shell_help, "list all commands");
    simple_shell_register_command("clear", simple_shell_clear, "clear the screen");
}

// 列出已注册的命令及说明
static i32 simple_shell_help(i32 argc, char *argv[])
{
    simple_shell_t *shell = simple_shell_get_current();
    if (!shell) return -1;
    for (i32 i = 0; i < g_simple_shell_cmd_count; ++i) {
        if (g_simple_shell_cmds[i].desc)
            simple_shell_print(shell, "%-12s - %s\r\n", g_simple_shell_cmds[i].name, g_simple_shell_cmds[i].desc);
        else
            simple_shell_print(shell, "%-12s\r\n", g_simple_shell_cmds[i].name);
    }
    return 0;
}
...
static i32 simple_shell_clear(i32 argc, char *argv[])
{
    ...
    simple_shell_print(shell, "\033[2J\033[H");
    return 0;
}

simple_shell_t* simple_shell_get_current(void)
{
    return g_simple_shell_current;
}

i32 simple_shell_register_command(const char *name, i32 (*func)(i32, char **), const char *desc)
{
    if (g_simple_shell_cmd_count >= SIMPLE_SHELL_MAX_CMD) return -1;
    g_simple_shell_cmds[g_simple_shell_cmd_count].name = name;
    g_simple_shell_cmds[g_simple_shell_cmd_count].func = func;
    g_simple_shell_cmds[g_simple_shell_cmd_count].desc = desc;
    g_simple_shell_cmd_count++;
    return 0;
}
...
{
    static char localbuf[SIMPLE_SHELL_LINE_BUFFER];
    ...
    if (argc == 0) return -1;
    for (i32 i = 0; i < g_simple_shell_cmd_count; ++i) {
        if (str_is_equal(g_simple_shell_cmds[i].name, argv[0])) {
            return g_simple_shell_cmds[i].func(argc, argv);
        }
    }
    if (g_simple_shell_current && g_simple_shell_current->write) {
        g_simple_shell_current->write("Command not Found\r\n", 18);
    }
    return -1;
}
...
void simple_shell_handler(simple_shell_t *shell, char data)
{
    ...
    if (data == '\r' || data == '\n') {
        ...
        if (idx > 0) {
            simple_shell_run_command_by_name(shell->buffer);
            idx = 0;
        }
...
void simple_shell_hexdump(simple_shell_t *shell, u32 addr, const u8 *data, u32 len)
{
    ...
    for (u32 i = 0; i < len; i += 16) {
        ...
        simple_shell_print(shell, "%s", line);
    }
}
