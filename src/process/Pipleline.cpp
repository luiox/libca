...
CAnonymousPipeServer::CAnonymousPipeServer(const PIPECLIENTINFO* pClientInfo)
    : m_hWrite(NULL)
    , m_hRead(NULL)
...
BOOL CAnonymousPipeServer::Create()
{
    ...
    //    USES_CONVERSION;
    BOOL bRet = CreateProcess(m_pClientInfo->m_sAppPath,
                              const_cast<TCHAR*>(m_pClientInfo->m_sCmdLine),
                              NULL,
                              NULL,
                              TRUE,
                              0,
...
BOOL CNamedPipe::Create(LPCTSTR lpszName, DWORD dwOpenMode, DWORD dwPipeMode, DWORD dwMaxInstances,
                        ...
                        LPSECURITY_ATTRIBUTES lpSecurityAttributes)
{
    ...
    m_hPipe = ::CreateNamedPipe(pszPipeName,
                                dwOpenMode,
                                dwPipeMode,
                                dwMaxInstances,
                                dwOutBufferSize,
                       ...
                       DWORD& dwNumberOfBytesWritten, LPOVERLAPPED lpOverlapped)
{
    ...
    BOOL bSuccess = ::WriteFile(
        m_hPipe, lpBuffer, dwNumberOfBytesToWrite, &dwNumberOfBytesWritten, lpOverlapped);
...
BOOL CNamedPipe::Write(LPCVOID lpBuffer, DWORD dwNumberOfBytesToWrite, LPOVERLAPPED lpOverlapped,
                       LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    assert(m_hPipe != NULL);   // Pipe must be open
    BOOL bSuccess =
        ::WriteFileEx(m_hPipe, lpBuffer, dwNumberOfBytesToWrite, lpOverlapped, lpCompletionRoutine);
...
BOOL CNamedPipe::Read(LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, DWORD& dwNumberOfBytesRead,
                      LPOVERLAPPED lpOverlapped)
{
    assert(m_hPipe != NULL);   // Pipe must be open
    BOOL bSuccess =
        ::ReadFile(m_hPipe, lpBuffer, dwNumberOfBytesToRead, &dwNumberOfBytesRead, lpOverlapped);
...
BOOL CNamedPipe::Read(LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, LPOVERLAPPED lpOverlapped,
                      LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine)
{
    assert(m_hPipe != NULL);   // Pipe must be open
    BOOL bSuccess =
        ::ReadFileEx(m_hPipe, lpBuffer, dwNumberOfBytesToRead, lpOverlapped, lpCompletionRoutine);
                      ...
                      DWORD& dwTotalBytesAvail, DWORD& dwBytesLeftThisMessage)
{
    ...
    BOOL bSuccess = ::PeekNamedPipe(
        m_hPipe, lpBuffer, dwBufferSize, &dwBytesRead, &dwTotalBytesAvail, &dwBytesLeftThisMessage);
...
{
    assert(m_hPipe != NULL);   // Pipe must be open
    BOOL bSuccess = ::TransactNamedPipe(m_hPipe,
                                        lpInBuffer,
                                        dwInBufferSize,
                                        lpOutBuffer,
                                        dwOutBufferSize,
...
DWORD CNamedPipe::GetMaxCollectionCount() const
{
    ...
    BOOL  bSuccess =
        ::GetNamedPipeHandleState(m_hPipe, NULL, NULL, &dwMaxCollectionCount, NULL, NULL, 0);
...
DWORD CNamedPipe::GetCollectionTimeout() const
{
    ...
    BOOL  bSuccess =
        ::GetNamedPipeHandleState(m_hPipe, NULL, NULL, NULL, &dwCollectDataTimeout, NULL, 0);
...
std::wstring CNamedPipe::GetClientUserName() const
{
    assert(m_hPipe != NULL);   // Pipe must be open
#ifdef _DEBUG
    BOOL bServerPipe;
    ...
    BOOL  bSuccess =
        ::GetNamedPipeHandleState(m_hPipe, NULL, NULL, NULL, NULL, pszUserName, _MAX_PATH);
    std::wstring sName;
                      ...
                      DWORD& dwBytesRead, DWORD dwTimeOut)
{
    ...
    CString sPipeName;
    sPipeName.Format(_T("\\\\%s\\PIPE\\%s"), lpszServerName, lpszPipeName);
    BOOL bSuccess = ::CallNamedPipe(sPipeName,
                                    lpInBuffer,
                                    dwInBufferSize,
                                    lpOutBuffer,
                                    dwOutBufferSize,
