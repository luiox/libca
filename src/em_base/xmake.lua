target("libca.em_base")
    set_kind("object")
    set_group("em")
    add_files("**.c")

target("libca.em_base_static")
    set_kind("static")
    set_group("em")
    add_files("**.c")

local em_base_test_dir = "$(projectdir)/tests/em_base"

-- datatype的单元测试
target("test-datatype")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("datatype.c", path.join(em_base_test_dir, "test_datatype.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })

-- debug的单元测试
target("test-debug")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("debug.c", path.join(em_base_test_dir, "test_debug.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })

-- compiler_compat的单元测试
target("test-compiler_compat")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("compiler_compat.c", path.join(em_base_test_dir, "test_compiler_compat.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })

-- macro_util的单元测试
target("test-macro_util")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("macro_util.c", path.join(em_base_test_dir, "test_macro_util.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })

-- string_util的单元测试（标准库版本，默认）
target("test-string_util_std")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("string_util.c", path.join(em_base_test_dir, "test_string_util.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })
    -- USE_CUSTOM_STRING_UTIL_IMPL 默认为 0，使用标准库内联实现

-- string_util的单元测试（自定义实现版本）
target("test-string_util_custom")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("string_util.c", path.join(em_base_test_dir, "test_string_util.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })
    add_defines("USE_CUSTOM_STRING_UTIL_IMPL=1")

-- memory_util的单元测试（标准库版本，默认）
target("test-memory_util_std")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("memory_util.c", path.join(em_base_test_dir, "test_memory_util.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })
    -- USE_CUSTOM_MEMORY_UTIL_IMPL 默认为 0，使用标准库内联实现

-- memory_util的单元测试（自定义实现版本）
target("test-memory_util_custom")
    set_kind("binary")
    add_includedirs(".", em_base_test_dir)
    add_files("memory_util.c", path.join(em_base_test_dir, "test_memory_util.c"))
    add_rules("em_test", { test_enable = true, use_default_main = true })
    add_defines("USE_CUSTOM_MEMORY_UTIL_IMPL=1")


