...
namespace libca::utility {
    ...
    void CommandLineParser::addOption(OptionType type, std::string shortName, std::string longName, std::string description, std::function<void(std::string)> handler)
    {
        m_options.push_back({type, shortName, longName, description, handler});
    }
    void CommandLineParser::addSubCommand(std::string name, std::string description, std::function<void(std::string)> handler)
    {
