...
bool Connection::connect(std::string ip, uint32_t port, std::string username, std::string password,
                         std::string dbname)
{
    MYSQL* mysql = mysql_real_connect(
        m_mysql, ip.c_str(), username.c_str(), password.c_str(), dbname.c_str(), port, nullptr, 0);
...
void Connection::refeshAliveTime()
{
    m_aliveTime = clock();
}
